...
Begin VB.Form RptSelDB 
   ...
   End
End
Attribute VB_Name = "RptSelDB"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of Rptseldb.frm on Wed 6/17/09 @ 12:56 P
...
'
' File Name: RptSelDB.Frm   Demo Bar Summary report
'
' Release: 5.0
'
' Description:
...
Private Sub cmcGen_Click()
    ...
    For ilJobs = ilStartJobNo To ilNoJobs Step 1
        ...
        If Not gGenReportDB() Then
            ...
            Exit Sub
        End If
        ilRet = gCmcGenDB(ilListIndex, imGenShiftKey)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Erase tgRptSelDBSalespersonCode
    Erase tgRptSelDBAgencyCode
    Erase tgRptSelDBAdvertiserCode
    Erase tgRptSelDBNameCode
    Erase tgRptSelDBBudgetCode
    'Erase tgMultiCntrCode
    'Erase tgManyCntCode
    Erase tgRptSelDBDemoCode
    'Erase tgSOCode
    PECloseEngine
    
    gChDrDir        '11/2/10: Handle case where printing directly to abode and rerouting abode output to different Drive/Path
    
    Set RptSelDB = Nothing   'Remove data segment
...
'*******************************************************
    ...
    tmCbfSrchKey.lGenTime = lgNowTime       '10-20-01
...
Private Sub mInit()
    ...
    imAllClicked = False
    imSetAll = True
    'gCenterStdAlone RptSelDB
    'RptSelDB.Move -90, -90, 30, 30      'make form small and out of the way so its not seen
    RptSelDB.Move -330, -330, 30, 30      'make form small and out of the way so its not seen
End Sub
'*******************************************************
'*                                                     *
...
Private Sub mInitReport()
    ...
    'cbcFileType.ListIndex = 0
    ' Dan M 11/30/11 snapshot from proposal/orders comes here via cSnapPrt...and cannot allow gPopExportTypes to happen
    Dim ilPreviousSelected As Integer
    ilPreviousSelected = imFTSelectedIndex
    ilRet = gPopExportTypes(cbcFileType)      '10-20-01
    imFTSelectedIndex = ilPreviousSelected
    ...
    mSetCommands
    Screen.MousePointer = vbDefault
'    gCenterModalForm RptSelDB
End Sub
'************************************************************
'*                                                          *
'*      Procedure Name:mParseCmmdLine                       *
'*                                                          *
'*             Created:6/11/99       By:D. Hosaka           *
'*            Modified:              By:                    *
'*                                                          *
'*            Comments: Parse command line                  *
'*
'*            Special mParseCmmdLine for "Demo Bar" process *
'*            Assumes that only the Demo Bar Snapshot       *
'*            come thru here                                *
'*                                                          *
'************************************************************
Private Sub mParseCmmdLine()
    ...
    '    slCommGenDate = "7/31/98"
    '    slCommGenTime = "12M"
    '    slInclRates = "Y"
    '    slSummaryID = "5"       'default to Line summary
    ...
    '    '3rd parm:  Gen Date
    '    '4th parm:  Gen Time
    '    '5th parm:  Summary ID:  5= line summary, 6 = week summary, 7 = qtr summary, 8 = vehicle summary , 9 = daypart summary
    '    slCommand = "Logs^Test\Guide\" & slCommGenDate & "\" & slCommGenTime & "\" & slSummaryID & "\" & slInclRates & "\" & slDisplayPrint
    '    imShowHelpmsg = False
    'Else
    ...
    'End If
    'gInitStdAlone RptSelDB, slStr, ilTestSystem
    ...
    ilRet = gParseItem(slCommand, 5, "\", slStr)                'Summary ID #
    igSummaryID = Val(Trim$(slStr))
    ...
    Else
        ...
        ilRet = gParseItem(slCommand, 8, "\", slStr)               'Save File Index
        imFTSelectedIndex = Val(slStr)
        'Dan M 11/29/11 I want to return this: D:\csi\V81\export\here.pdf but I am getting D:
        'if you don't browse to a new folder, below would work properly because the segment would be "here.pdf"
        'ilRet = gParseItem(slCommand, 9, "\", slStr)               'Save File Name
        'find where the file name begins by reconstructing some of the old command line
        slStr = igSummaryID & "\" & sgInclRates & "\" & igOutputTo & "\" & imFTSelectedIndex & "\"
        ilRet = InStr(1, slCommand, slStr)
        If ilRet > 0 Then
            slStr = Mid(slCommand, ilRet + Len(slStr))
        'this should never happen--let it try the old way
        Else
            ilRet = gParseItem(slCommand, 9, "\", slStr)
        End If
