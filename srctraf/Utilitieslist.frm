VERSION 5.00
Begin VB.Form UtilitiesList 
   Appearance      =   0  'Flat
   Caption         =   "CSI Utilities"
   ClientHeight    =   5085
   ClientLeft      =   585
   ClientTop       =   2880
   ClientWidth     =   3885
   ...
   Begin VB.PictureBox plcExport 
      ...
      Top             =   210
      Width           =   3570
      Begin VB.ListBox lbcUtilities 
         Appearance      =   0  'Flat
         Height          =   3810
         ItemData        =   "Utilitieslist.frx":08CA
         Left            =   60
         List            =   "Utilitieslist.frx":08CC
         Sorted          =   -1  'True
         TabIndex        =   2
         Top             =   75
         Width           =   3405
   ...
   End
End
Attribute VB_Name = "UtilitiesList"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of UtilitiesList.frm on Wed 6/17/09 @ 12:56 PM
...
Private Sub cmcDone_Click()
    ...
    If lbcUtilities.ListIndex < 0 Then
        Exit Sub
    End If
    
    mCheckGG
    ilIndex = lbcUtilities.ItemData(lbcUtilities.ListIndex)
    ...
    End If
    sgCommandStr = slStr
    Select Case lbcUtilities.ItemData(lbcUtilities.ListIndex)
        Case 0  'SSF Check
            mSSFCheck
        Case 1  'SMF Check
            mSMFCheck
        Case 2  'Sport Contract Check
            mSportCheck
        Case 3  'CFF Check
            mCffCheck
        Case 4  'Contract Swap Check
            mContractSwapCheck
        Case 5  'Invoice Check
            mInvCheck
        Case 6  'Acquisition Check
            mAcquisitionCheck
        Case 7  'Set Copy Inventory Dates
            mSetInvDate
        Case 8  'View SQL
            mViewSql
        Case 9  'Send to EDS
            mSendToEDS
        Case 10 'Compare ACT1
            mCompareACT1
        Case 11 'Average to Pay
            mAvgToPay
    End Select
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If igLogActivityStatus = 32123 Then
        ...
        gUserActivityLog "", ""
    End If
    Set UtilitiesList = Nothing   'Remove data segment
...
Private Sub lbcUtilities_Click()
    Dim slStr As String
    
End Sub

Private Sub lbcUtilities_DblClick()
    cmcDone_Click
End Sub

Private Sub lbcUtilities_GotFocus()
    If imFirstFocus Then
        imFirstFocus = False
    End If
End Sub
...
'
    ...
    'UtilitiesList.Height = cmcDone.Top + 3 * cmcDone.Height '/ 3
    gCenterStdAlone UtilitiesList
    'UtilitiesList.Show
    Screen.MousePointer = vbHourglass
'    mInitDDE
    smScreenCaption = "Utilities"
    ...
    plcScreen_Paint
'    gCenterModalForm UtilitiesList
...
Private Sub mPopulate()
    ...
    lbcUtilities.Clear
    lbcUtilities.AddItem "SSF Check"
    lbcUtilities.ItemData(lbcUtilities.NewIndex) = 0
    lbcUtilities.AddItem "SMF Check"
    lbcUtilities.ItemData(lbcUtilities.NewIndex) = 1
    lbcUtilities.AddItem "Sport Contract Check"
    lbcUtilities.ItemData(lbcUtilities.NewIndex) = 2
    lbcUtilities.AddItem "CFF Check"
    lbcUtilities.ItemData(lbcUtilities.NewIndex) = 3
    lbcUtilities.AddItem "Contract Swap Check"
    lbcUtilities.ItemData(lbcUtilities.NewIndex) = 4
    lbcUtilities.AddItem "Invoice Check"
    lbcUtilities.ItemData(lbcUtilities.NewIndex) = 5
    For ilVff = LBound(tgVff) To UBound(tgVff) Step 1
        If tgVff(ilVff).sPostLogSource = "S" Then
            lbcUtilities.AddItem "Acquisition Check"
            lbcUtilities.ItemData(lbcUtilities.NewIndex) = 6
            Exit For
        End If
    Next ilVff
    lbcUtilities.AddItem "Set Copy Inventory Dates"
    lbcUtilities.ItemData(lbcUtilities.NewIndex) = 7
    slPath = sgExePath & "ViewSql.exe"
    If gFileExist(slPath) = FILEEXISTS Then
        lbcUtilities.AddItem "View SQL"
        lbcUtilities.ItemData(lbcUtilities.NewIndex) = 8
    End If
    If (Asc(tgSaf(0).sFeatures2) And EMAILDISTRIBUTION) = EMAILDISTRIBUTION Then
        lbcUtilities.AddItem "Send to EDS"
        lbcUtilities.ItemData(lbcUtilities.NewIndex) = 9
        bgEDSIsActive = True
    Else
        bgEDSIsActive = False
    End If
    If bgStationAreVehicles Then
        lbcUtilities.AddItem "Compare ACT1 Line-up to Packages"
        lbcUtilities.ItemData(lbcUtilities.NewIndex) = 10
    End If
    lbcUtilities.AddItem "Days to Pay Check"
    lbcUtilities.ItemData(lbcUtilities.NewIndex) = 11
...
Private Sub mSportCheck()
    On Error Resume Next
    SportChk.Show vbModal
End Sub
Private Sub mCffCheck()
    Dim slStr As String
    If igTestSystem Then
        slStr = "Traffic^Test\" & sgUserName & "\" & Trim$(Str$(CALLNONE))
    Else
    ...
    gChDrDir
End Sub
Private Sub mContractSwapCheck()
    ContractSwapCheck.Show vbModal
End Sub
Private Sub mInvCheck()
    Dim slStr As String
    If igTestSystem Then
        slStr = "Traffic^Test\" & sgUserName & "\" & Trim$(Str$(CALLNONE))
    Else
    ...
    gChDrDir
End Sub
Private Sub mAcquisitionCheck()
    AcqCheck.Show vbModal
End Sub

Private Sub mSetInvDate()
    SetCopyDates.Show vbModal
End Sub

Private Sub mViewSql()
    '6240
    Dim slPath As String
...
Private Sub mCompareACT1()
    StdPkgVsACT1.Show vbModal
End Sub

Private Sub mAvgToPay()
    AvgToPay.Show vbModal
End Sub
