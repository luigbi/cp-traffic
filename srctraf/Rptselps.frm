...
Begin VB.Form RptSelPS 
   ...
   End
End
Attribute VB_Name = "RptSelPS"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of Rptselps.frm on Wed 6/17/09 @ 12:56 P
...
'
' File Name: RptSelPS.Frm    Projection Scenarios
'               5/28/99 Allow 10 char input date MM/dd/yyyy (from 8-mm/dd/yy)
'
' Release: 1.0
'
' Description:
...
Private Sub cmcGen_Click()
    ...
    For ilJobs = ilStartJobNo To ilNoJobs Step 1
        igJobRptNo = ilJobs
        If Not gOpenPrtJob("PJScene.Rpt") Then
            igGenRpt = False
            ...
            Exit Sub
        End If
        ilRet = gCmcGenPS(imGenShiftKey, smLogUserCode)
        If ilRet = -1 Then
            igGenRpt = False
            frcOutput.Enabled = igOutput
            frcCopies.Enabled = igCopies
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Erase tgCSVNameCode
    Erase tgRptSelBudgetCodePS
    Erase tgClfPS
    Erase tgCffPS
    Erase imCodes
    PECloseEngine
    
    Set RptSelPS = Nothing   'Remove data segment
...
Private Sub mAskEffDate()
    ...
    edcSelCFrom.MaxLength = 10  '8   5/28/99 allow m/d/yyyy input
...
'
    Dim ilRet As Integer
    ilRet = gPopVehBudgetBox(RptSelPS, 2, 0, 1, lbcSelection(0), tgRptSelBudgetCodePS(), sgRptSelBudgetCodeTagPS)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mBudgetPopErr
        gCPErrorMsg ilRet, "mBudgetPopErr (gPopVehBudgetBox)", RptSelPS
        On Error GoTo 0
    End If
    Exit Sub
mBudgetPopErr:
...
Dim slStr As String
    ...
    RptSelPS.Caption = smSelectedRptName & " Report"
...
Private Sub mInitReport()
    ...
    For ilIndex = 0 To lbcSelection(0).ListCount - 1 Step 1
        slStr = lbcSelection(0).List(ilIndex)
        lbcSelection(1).AddItem slStr               'lbcselection(2) = comparison budgets
    Next ilIndex
    ...
    laclbcName(1).Caption = "Forecast Names"
    laclbcName(0).Move 15, 0
    laclbcName(0).Width = 2205
    lbcSelection(0).Move 15, laclbcName(0).Height + 30, 4380, 1500
    lbcSelection(1).Move lbcSelection(0).Left, lbcSelection(0).Top + lbcSelection(0).Height + 300, 4380, 1470
    laclbcName(1).Move lbcSelection(0).Left, lbcSelection(1).Top - laclbcName(0).Height - 30, 2190
    ckcAll.Visible = False
    pbcOption.Visible = True
    ...
    End If
        ' dan M 7-15-08 hard cost and NTR options, and contract selectivity
    mAddNTRHardCost
    lacSelC2.Move 120, 30
    edcSelC2.Left = lacSelC2.Left + lacSelC2.Width
    edcSelC2.Visible = True
    plcSelC2.Move 0, plcSelC3.Top + plcSelC3.Height
...
Private Sub mParseCmmdLine()
    ...
    'End If
    'gInitStdAlone RptSelPS, slStr, ilTestSystem
    'If igStdAloneMode Then
    '    smSelectedRptName = "Projection Scenarios"
...
'
    ...
    ilRet = gPopSalespersonBox(RptSelPS, 0, True, True, lbcSelection, tgSalesperson(), sgSalespersonTag, igSlfFirstNameFirst)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mSPersonPopErr
        gCPErrorMsg ilRet, "mSPersonPop (gPopSalespersonBox)", RptSelPS
        On Error GoTo 0
    End If
    Exit Sub
mSPersonPopErr:
...
Private Sub mAddNTRHardCost()
    ...
    plcSelC3.Move 150, plcSelC1.Top + plcSelC1.Height + 30
