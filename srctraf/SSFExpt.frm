...
Dim hmToExport As Integer   'To file hanle
...
Private Sub cmcProcess_Click()
    ...
    If rbcEditReplace(0).Value Or rbcEditReplace(1).Value Then 'Show or Fix by Typing
        ...
        gdcData.Col = 0
        gdcData.Row = 0
        gdcData.ColWidth(0) = SSFExpt.TextWidth(" Display #")
        ...
        For ilLoop = 0 To lbcEditReplace(2).ListCount - 2 Step 1
            ...
            gdcData.Col = ilCol
            gdcData.Row = 0
            gdcData.ColWidth(ilCol) = SSFExpt.TextWidth(Trim$(slFieldName) & "   ")
    ...
    ElseIf rbcEditReplace(2).Value Then 'Fix by Mass Substitution
        ...
        cmcGCancel.Enabled = False
        cmcGDone.Enabled = True
    ElseIf rbcEditReplace(3).Value Then 'Export
        mnuFilePrint.Enabled = False
        mnuFileNew.Enabled = False
        mnuFileOpen.Enabled = False
        plcSelection.Visible = False
        ...
        ilRet = 0
        On Error GoTo cmcProcessErr:
        hmToExport = FreeFile
        Open smDBPath & "SSF.DAT" For Binary Access Write As hmToExport
        If ilRet <> 0 Then
            Screen.MousePointer = vbDefault
            MsgBox "Open " & " SSF.DAT error: " & Str(ilRet), vbOkOnly + vbCritical + vbApplicationModal, "Error"
            Exit Sub
        End If
        mObtainRecords 2
        cmcGCancel.Enabled = False
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set SSFExpt = Nothing   'Remove data segment
    btrStopAppl
    End
End Sub
                ...
                For ilLoop = 1 To tmSsf.iCount Step 1
                   ...
                   LSet tmProg = tmSsf.tPas(ADJSSFPASBZ + ilLoop)
                    ...
                    ElseIf (tmProg.iRecType And &HF) = 10 Then    'Spot
                       LSet tmSpot = tmSsf.tPas(ADJSSFPASBZ + ilLoop)
                        gdcSub.Col = 0
                        gdcSub.Row = ilRow
                        gdcSub.Text = "Spot"
                        gdcSub.Col = 1
                        ...
                        gdcSub.Text = Format$(llValue, "m/d/yy")
                        gdcSub.Col = 3
                        gdcSub.Row = ilRow
                    ...
                    ElseIf (tmProg.iRecType And &HF) = 11 Then    'BBSpot
                        tmBBSpot = tmSsf.tPas(ADJSSFPASBZ + ilLoop)
                        gdcSub.Col = 0
                        gdcSub.Row = ilRow
                        gdcSub.Text = "BBSpot"
                        gdcSub.Col = 1
...
Private Sub lbcFile_Click()
    ...
    If lbcFile.ListIndex >= 0 Then
        ...
        For ilLoop = 0 To UBound(tmDDFFile) - 1 Step 1
            ...
            End If
        Next ilLoop
        If (Trim$(slName) = "SSF Spot Summary") Or (Trim$(slName) = "SSF_Spot_Summary") Then
...
Private Sub mFieldBuild(ilFileID As Integer, tlDDFField() As DDFFIELD)
    ...
    If ilRet <> BTRV_ERR_NONE Then
        ...
        Exit Sub
    End If
    lmActualRecLen = Len(tlDDFField(ilUpper)) 'Len(tmRec)
    imBaseRecLen = lmActualRecLen
    tmFieldSrchKey1.iFileID = ilFileID
    'ilRet = btrGetGreaterOrEqual(hmFile, tlDDFField(ilUpper), lmActualRecLen, tmFieldSrchKey1, INDEXKEY1, BTRV_LOCK_NONE)
    ...
    Do While (ilRet = BTRV_ERR_NONE)
        ...
        End If
        mGetFieldFields
        tlDDFField(ilUpper) = tmGetDDFFIELD
        If (tlDDFField(ilUpper).iFileID <> ilFileID) Then
            Exit Do
        End If
...
'
    ...
    'plcGauge.Move SSFExpt.Width / 2 - plcGauge.Width / 2
    'cmcFix.Move SSFExpt.Width / 2 - cmcFix.Width / 2
    'cmcCancel.Move SSFExpt.Width / 2 - cmcCancel.Width / 2
        ...
        If ((ilRecIndex1 = imRecMaxUpper1) And (ilRecIndex2 < imRecMaxUpper2)) Or ((ilRecIndex1 < imRecMaxUpper1) And (ilRecIndex2 <= 32000)) Then
            Do
                ...
                For ilLoop = 0 To lbcEditReplace(2).ListCount - 2 Step 1
                    ...
                    If Left$(smFieldValue, 1) = """" Then
                        smFieldValue = Mid$(smFieldValue, 2)
                    End If
                    If right$(smFieldValue, 1) = """" Then
...
Private Sub mObtainRecords(ilForMassChg As Integer)
    ...
    ilKeyIndex = Val(edcSortKey.Text)
    imRecUpper1 = 1
    imRecUpper2 = 1
    imRecMaxUpper1 = 1
    imRecMaxUpper2 = 1
    ...
    Else
        ...
        Do
            ...
            If (ilForMassChg = 0) And (ilRet = 0) Then
    ...
    If (imVarLenRec) Or (Trim$(lbcFile.List(lbcFile.ListIndex)) = "SSF Spot Summary") Or (Trim$(lbcFile.List(lbcFile.ListIndex)) = "SSF_Spot_Summary") Then
        ...
        Do While (ilRet = BTRV_ERR_NONE)
            ...
            If mRecordSelected() Then
                If ilForMassChg = 2 Then
                    llNoShown = llNoShown + 1
                    Put #hmToExport, , lmActualRecLen
                    For llLoop = 1 To lmActualRecLen Step 1
                    
                        Put #hmToExport, , tmRec.sChar(llLoop)
                    Next llLoop
                Else
                    llNoShown = llNoShown + 1
                    ...
                    If Len(slLine) > 0 Then
                        ...
                        ElseIf ilImportArea = 7 Then    'Fields
                            If Not ilGetDescriptionOnly Then
                                ...
                                End If
                            End If
                        ElseIf ilImportArea = 8 Then    'Run
                            If Not ilGetDescriptionOnly Then
...
Private Sub rbcEditReplace_Click(Index As Integer)
    ...
    'End of coded added
    If Value Then
        If (Index = 2) And (Not imPasswordAsked) Then
            sgDBPath = lbcDataPath.Path & "\"
            CSIPass.Show vbModal
            If Not igPasswordOk Then
                rbcEditReplace(2).Enabled = False
        ...
        ElseIf Index = 1 Then
            ...
            cmcEditReplace(1).Enabled = True
        ElseIf Index = 3 Then
            lacEditReplace(0).Caption = "Export SSF"
            lacEditReplace(1).Enabled = False
            edcReplaceValue.Enabled = False
            cmcEditReplace(1).Enabled = True
            cmcEditReplace_Click 1
        Else
...
Private Sub vbcData_Change()
    ...
    End If
    ilRow = 1
    llStartRec = imVBCAdjust * (vbcData.Value - 1) + 1
