Attribute VB_Name = "RPTVFYPA"
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of Rptvfypa.bas on Wed 6/17/09 @ 12:56 P
...
'*                                                     *
'*******************************************************
Function gCmcGenPA(ilGenShiftKey As Integer, slLogUserCode As String) As Integer
'
'   ilRet = gCmcGenPA()
'
...
'
    ...
    Dim slActDateTo As String
    Dim slEntDateFrom As String
    Dim slEntDateTo As String
    ...
    Dim llDate2 As Long
    gCmcGenPA = 0
'   12-16-19 change dates from edit box to csi calendar control
'    slActDateFrom = RptSelPA!edcSelCFrom.Text   'Active From Date
    slActDateFrom = RptSelPA!CSI_CalFrom.Text   'Active From Date
    If slActDateFrom = "" Then
        slActDateFrom = "1/1/1970"
    Else
        If Not gValidDate(slActDateFrom) Then
            mReset
            RptSelPA!CSI_CalFrom.SetFocus
            Exit Function
        End If
    End If
'    slActDateTo = RptSelPA!edcSelCFrom1.Text    'Active To Date
    slActDateTo = RptSelPA!CSI_CalTo.Text    'Active To Date
    If slActDateTo = "" Then
        slActDateTo = "12/31/2069"
    Else
        If Not gValidDate(slActDateFrom) Then
            mReset
            RptSelPA!CSI_CalTo.SetFocus
            Exit Function
        End If
    End If
    llDate1 = gDateValue(slActDateFrom)         'Verify that the From Date < than the To date
    llDate2 = gDateValue(slActDateTo)
...
'    slEntDateFrom = RptSelPA!edcSelCTo.Text     'Entered From Date
    slEntDateFrom = RptSelPA!CSI_CalFrom1.Text     'Entered From Date
    If slEntDateFrom = "" Then
        slEntDateFrom = "1/1/1970"
    Else
        If Not gValidDate(slActDateFrom) Then
            mReset
            RptSelPA!CSI_CalFrom1.SetFocus
            Exit Function
        End If
    End If
'    slEntDateTo = RptSelPA!edcSelCTo1.Text      'Entered To Date
    slEntDateTo = RptSelPA!CSI_CalTo1.Text      'Entered To Date
    If slEntDateTo = "" Then
        slEntDateTo = "12/31/2069"
    Else
        If Not gValidDate(slEntDateTo) Then
            mReset
            RptSelPA!CSI_CalTo1.SetFocus
            Exit Function
        End If
    End If
    llDate1 = gDateValue(slEntDateFrom)            'Verify that the From Date < than the To date
    llDate2 = gDateValue(slEntDateTo)
    If llDate1 > llDate2 Then
        mReset
        RptSelPA!CSI_CalFrom1.SetFocus
        Exit Function
...
Sub mReset()
    igGenRpt = False
    RptSelPA!frcOutput.Enabled = igOutput
    RptSelPA!frcCopies.Enabled = igCopies
    'rptselpa!frcWhen.Enabled = igWhen
    RptSelPA!frcFile.Enabled = igFile
    RptSelPA!frcOption.Enabled = igOption
    'rptselpa!frcRptType.Enabled = igReportType
    Beep
End Sub
'
'
...
Dim slStr As String
        ...
        mWeekQtrHdr = True
        ilYear = RptSelPA!edcSelCTo.Text                'starting year
        ilMonth = RptSelPA!edcSelCTo1.Text              'month
