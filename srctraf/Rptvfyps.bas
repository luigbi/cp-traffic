Attribute VB_Name = "RPTVFYPS"
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of Rptvfyps.bas on Wed 6/17/09 @ 12:56 P
...
'*                                                     *
'*******************************************************
Function gCmcGenPS(ilGenShiftKey As Integer, slLogUserCode As String) As Integer
'
'
'
'   ilRet (O)-  -1=Terminate
...
'
    ...
    slDate = RptSelPS!edcSelCFrom.Text
    If Not gValidDate(slDate) Then
        mReset
        RptSelPS!edcSelCFrom.SetFocus
        Exit Function
    End If
    slStr = RptSelPS!edcSelCTo.Text                 'entered year
    igYear = gVerifyYear(slStr)
    If igYear = 0 Then
        mReset
        RptSelPS!edcSelCTo.SetFocus                 'invalid year
        gCmcGenPS = -1
        Exit Function
    End If
    slStr = RptSelPS!edcSelCTo1.Text                  'edit qtr
    ilRet = gVerifyInt(slStr, 1, 4)
    If ilRet = -1 Then
        mReset
        RptSelPS!edcSelCTo1.SetFocus                 'invalid qtr
        gCmcGenPS = -1
        Exit Function
    End If
    ...
    If Not mWeekQtrHdr(slDate) Then           'pass year/month as formula to crystal report
        ...
        Exit Function
    End If
           'dan M 7-15-08 NTR/Hard Cost option
    slStr = ""
    If RptSelPS!ckcSelCInclude(0).Value = 1 Or RptSelPS!ckcSelCInclude(1).Value = 1 Then
        slStr = slStr & "'With"
...
Sub mReset()
    igGenRpt = False
    RptSelPS!frcOutput.Enabled = igOutput
    RptSelPS!frcCopies.Enabled = igCopies
    'RptSelPS!frcWhen.Enabled = igWhen
    RptSelPS!frcFile.Enabled = igFile
    RptSelPS!frcOption.Enabled = igOption
    'RptSelPS!frcRptType.Enabled = igReportType
    Beep
End Sub
' *********************************************************************
'
...
Dim slStr As String
        ...
        mWeekQtrHdr = True
        ilYear = RptSelPS!edcSelCTo.Text                'starting year
        ilMonth = RptSelPS!edcSelCTo1.Text              'month
