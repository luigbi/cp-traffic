...
Begin VB.Form BudActA 
   ...
   End
End
Attribute VB_Name = "BudActA"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of Budacta.frm on Wed 6/17/09 @ 12:56 PM
...
Private Sub cmcApply_Click()
    ...
    ilHOType = 1
    sgCntrForDateStamp = ""
    ilRet = gObtainCntrForDate(BudActA, slStartDate, slEndDate, slStatus, slCntrType, ilHOType, tmChfAdvtExt())
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    btrDestroy hmClf
    btrDestroy hmCff
    Set BudActA = Nothing   'Remove data segment
...
'
    ...
    gBtrvErrorMsg ilRet, "mInit (btrOpen: Chf.Btr)", BudActA
    On Error GoTo 0
    imCHFRecLen = Len(tmChf)
    hmClf = CBtrvTable(ONEHANDLE)
    ilRet = btrOpen(hmClf, "", sgDBPath & "Clf.Btr", BTRV_OPEN_NORMAL, BTRV_OPEN_NONSHARE, BTRV_LOCK_NONE)
    On Error GoTo mInitErr
    gBtrvErrorMsg ilRet, "mInit (btrOpen: Clf.Btr)", BudActA
    On Error GoTo 0
    imClfRecLen = Len(tmClf)
    hmCff = CBtrvTable(ONEHANDLE)
    ilRet = btrOpen(hmCff, "", sgDBPath & "Cff.Btr", BTRV_OPEN_NORMAL, BTRV_OPEN_NONSHARE, BTRV_LOCK_NONE)
    On Error GoTo mInitErr
    gBtrvErrorMsg ilRet, "mInit (btrOpen: Cff.Btr)", BudActA
    On Error GoTo 0
    imCffRecLen = Len(tmCff)
    BudActA.Height = cmcApply.Top + 5 * cmcApply.Height / 3
    gCenterStdAlone BudActA
    imChgMode = False
    imBSMode = False
    imDateChgMode = False
