Attribute VB_Name = "RPTVFYUS"
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of Rptvfyus.bas on Wed 6/17/09 @ 12:56 P
...
'*                                                     *
'*******************************************************
Function gCmcGenUS(ilGenShiftKey As Integer, slLogUserCode As String) As Integer
'
'
'
'   ilRet (O)-  -1=Terminate
...
'
    ...
    gCmcGenUS = 0
        'dan M 6-27-08 NTR/Hard Cost option
    slStr = ""
    If RptSelUS!ckcSelCInclude(0).Value = 1 Or RptSelUS!ckcSelCInclude(1).Value = 1 Then
        slStr = slStr & "'With"
...
'    slDate = RptSelUS!edcSelCFrom.Text
    slDate = RptSelUS!CSI_CalFrom.Text          '12-13-19 use csi calendar control vs edit box
    If Not gValidDate(slDate) Then
        mReset
        RptSelUS!CSI_CalFrom.SetFocus
        Exit Function
    End If
    slStr = RptSelUS!edcSelCTo.Text                 'entered year
    igYear = gVerifyYear(slStr)
    If igYear = 0 Then
        mReset
        RptSelUS!edcSelCTo.SetFocus                 'invalid year
        gCmcGenUS = -1
        Exit Function
    End If
    slStr = RptSelUS!edcSelCTo1.Text                  'edit qtr
    ilRet = gVerifyInt(slStr, 1, 4)
    If ilRet = -1 Then
        mReset
        RptSelUS!edcSelCTo1.SetFocus                 'invalid qtr
        gCmcGenUS = -1
        Exit Function
    End If
...
Private Sub mReset()
    igGenRpt = False
    RptSelUS!frcOutput.Enabled = igOutput
    RptSelUS!frcCopies.Enabled = igCopies
    'RptSelUS!frcWhen.Enabled = igWhen
    RptSelUS!frcFile.Enabled = igFile
    RptSelUS!frcOption.Enabled = igOption
    'RptSelUS!frcRptType.Enabled = igReportType
    Beep
End Sub
' *********************************************************************
'
...
Dim slStr As String
        ...
        mWeekQtrHdr = True
        ilYear = RptSelUS!edcSelCTo.Text                'starting year
        ilMonth = RptSelUS!edcSelCTo1.Text              'month
