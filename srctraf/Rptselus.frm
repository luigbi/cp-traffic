...
Begin VB.Form RptSelUS 
   ...
   Begin VB.Frame frcOption 
      ...
      Begin VB.PictureBox pbcSelC 
         ...
         Begin VB.PictureBox plcSelC2 
            ...
            Top             =   1020
            Width           =   4380
            Begin VB.CheckBox ckcSelC2 
               Caption         =   "Include Salesperson Totals"
               ForeColor       =   &H80000008&
               Height          =   270
               Index           =   0
               Left            =   0
      ...
      Begin VB.PictureBox pbcOption 
         ...
         Begin VB.ListBox lbcSelection 
            ...
            Height          =   1290
            Index           =   1
            ItemData        =   "Rptselus.frx":0000
            Left            =   120
            List            =   "Rptselus.frx":0002
         ...
         Begin VB.ListBox lbcSelection 
            ...
            Height          =   1290
            Index           =   0
            ItemData        =   "Rptselus.frx":0004
            Left            =   120
            List            =   "Rptselus.frx":000B
            MultiSelect     =   2  'Extended
            TabIndex        =   35
            Top             =   330
            Width           =   4260
   ...
   End
End
Attribute VB_Name = "RptSelUS"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of Rptselus.frm on Wed 6/17/09 @ 12:56 P
...
'
' File Name: RptSelUS.Frm  Upfront/Scatter
'       5/28/99 allow 10 char input date mm/dd/yyyy
'
' Release: 1.0
'
' Description:
...
Dim imAllSlspClicked As Integer  'True=All box clicked (don't call ckcAll within lbcSelection)
...
Private Sub cmcGen_Click()
    ...
    For ilJobs = ilStartJobNo To ilNoJobs Step 1
        igJobRptNo = ilJobs
        If ckcSelC2(0).Value = vbChecked Then               'include slsp totals
            slReptName = "UpfScat2.rpt"
        Else
            slReptName = "UpfScat.Rpt"          'no slsp totals
        End If
        If Not gOpenPrtJob(slReptName) Then
            ...
            Exit Sub
        End If
        ilRet = gCmcGenUS(imGenShiftKey, smLogUserCode)
        If ilRet = -1 Then
            igGenRpt = False
            frcOutput.Enabled = igOutput
            frcCopies.Enabled = igCopies
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set RptSelUS = Nothing   'Remove data segment
...
Dim slStr As String
    ...
    RptSelUS.Caption = smSelectedRptName & " Report"
...
'    plcSelC2.Move 120, plcSelC1.Top + plcSelC2.Height
'    ckcSelC2(0).Move 0, 0
    pbcSelC.Move 90, 255, 4515, 3360

    gCenterStdAlone RptSelUS
End Sub
'*******************************************************
'*                                                     *
...
Private Sub mInitReport()
    ...
    mSalespersonPop lbcSelection(1)
...
'    lbcSelection.Move 15, ckcAll.Top + ckcAll.Height, 4380, 3000
    ...
    ' dan M 6-27-08 hard cost and NTR options
    mAddNTRHardCost
...
Private Sub mParseCmmdLine()
    ...
    'End If
    'gInitStdAlone RptSelUS, slStr, ilTestSystem
    'If igStdAloneMode Then
    '    smSelectedRptName = "Upfront/Scatter"
...
Private Sub mSalesOfficePop(lbcSelection As Control)
    ...
    ilRet = gPopOfficeSourceBox(RptSelUS, lbcSelection, tgSOCode(), sgSOCodeTag)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mSalesOfficePopErr
        gCPErrorMsg ilRet, "mSalesOfficePop (gPopOfficeSourceBox: Salesoffice)", RptSelUS
        On Error GoTo 0
    End If
    Exit Sub
mSalesOfficePopErr:
...
'*******************************************************
'*                                                     *
'*      Procedure Name:mSalespersonPop                 *
'*                                                     *
'*             Created:6/16/93       By:D. LeVine      *
'*            Modified:              By:               *
'*                                                     *
'*            Comments: Populate the selection slsp    *
'*                      combo box                      *
'*                                                     *
'*******************************************************
Private Sub mSalespersonPop(lbcSelection As Control)
    Dim ilRet As Integer
    ilRet = gPopSalespersonBox(RptSelUS, 0, True, True, lbcSelection, tgSalesperson(), sgSalespersonTag, igSlfFirstNameFirst)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mSalespersonPopErr
        gCPErrorMsg ilRet, "mSalespersonPop (gPopSalespersonBox)", RptSelUS
        On Error GoTo 0
    End If
    Exit Sub
mSalespersonPopErr:
...
'    If (edcSelCFrom.Text <> "") And (edcSelCTo.Text <> "") And (edcSelCTo1.Text <> "") Then
    If (CSI_CalFrom.Text <> "") And (edcSelCTo.Text <> "") And (edcSelCTo1.Text <> "") Then     '12-13-19 use csi calendar control
...
'plcSelC3.Move 150, plcSelC2.Top + plcSelC2.Height + 30
