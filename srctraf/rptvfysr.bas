Attribute VB_Name = "RptVfySR"
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of rptvfysr.bas on Fri 3/12/10 @ 11:00 A
...
Sub mReset()
    igGenRpt = False
    RptSelSR!frcOutput.Enabled = igOutput
    RptSelSR!frcCopies.Enabled = igCopies
    RptSelSR!frcFile.Enabled = igFile
    RptSelSR!frcOption.Enabled = igOption
    Beep
End Sub
'**************************************************************
'*                                                             *
'*      Procedure Name:gGenReportSR                            *
'*      Create Split Region report                             *
'*                                                             *
'*            Modified:              By:                       *
'*                                                             *
...
'*                    2 = successful Bridge                    *
'***************************************************************
Function gCmcGenSR(ilListIndex As Integer) As Integer
    ...
    If RptSelSR!ckcDormant.Value = vbChecked Then          'include dormants
        If Not gSetFormula("IncludeDormant", "'Dormant & Active '") Then
            gCmcGenSR = -1
            Exit Function
        End If
    Else                                        'Active regions only
        If Not gSetFormula("IncludeDormant", "'Active Only'") Then
            gCmcGenSR = -1
            Exit Function
        End If
    End If

    If ilListIndex = CR_SPLITREGION Then         'Split region report

        'send crystal whether its by Advertiser or Region
        If RptSelSR!rbcSortBy(0).Value Then          'Advertisr
        ...
        If RptSelSR!ckcDormant.Value = vbChecked Then          'include dormants
            If Not gSetFormula("IncludeDormant", "' '") Then
                gCmcGenSR = -1
                Exit Function
            End If
        Else                                        'Active regions only
        ...
        If RptSelSR!ckcInclStations.Value = vbChecked Then          ' detail
        ...
        slSelection = slSelection & " And {GRF_Generic_Report.grfPer1Genl} = 0"
    ...
    ElseIf ilListIndex = CR_COPYREGION Then
        ...
        'Split region & Copy region report both use Start/End dates
        'send formula to reports
        slStartDate = RptSelSR!edcDateFrom.Text
        slEndDate = RptSelSR!edcDateTo.Text
        If Trim$(slStartDate) = "" And Trim$(slEndDate) = "" Then
            slStr = "All Dates"
        ElseIf Trim$(slStartDate) = "" Then
            slEndDate = Format(gDateValue(slEndDate), "m/d/yy")
            slStr = "Thru " & slEndDate
        ElseIf Trim$(slEndDate) = "" Then
            slStartDate = Format(gDateValue(slStartDate), "m/d/yy")
            slStr = "From " & slStartDate
        Else            'both dates entered
            slStartDate = Format(gDateValue(slStartDate), "m/d/yy")
            slEndDate = Format(gDateValue(slEndDate), "m/d/yy")
            slStr = slStartDate & "-" & slEndDate
        End If
        If Not gSetFormula("DateRg", "'" & slStr & "'") Then
            gCmcGenSR = -1
            Exit Function
        End If
            
        'send crystal whether its by Advertiser or Region
        If RptSelSR!rbcCodeSort(0).Value Then          'Advertisr
            If Not gSetFormula("SortBy", "'A'") Then
                gCmcGenSR = -1
                Exit Function
            End If
        ElseIf RptSelSR!rbcCodeSort(1).Value Then   'region name
            If Not gSetFormula("SortBy", "'R'") Then
                gCmcGenSR = -1
                Exit Function
            End If
        ElseIf RptSelSR!rbcCodeSort(2).Value Then   'region code
            If Not gSetFormula("SortBy", "'C'") Then
                gCmcGenSR = -1
                Exit Function
            End If
        End If
...
'*******************************************************
Function gGenReportSR(ilListIndex As Integer) As Integer
Dim slDate As String
    ...
    slDate = RptSelSR!edcDateFrom.Text
    If slDate <> "" Then
        If Not gValidDate(slDate) Then
            mReset
            RptSelSR!edcDateFrom.SetFocus
            Exit Function
        End If
    End If
    slDate = RptSelSR!edcDateTo.Text
    If slDate <> "" Then
        If Not gValidDate(slDate) Then
            mReset
            RptSelSR!edcDateTo.SetFocus
            Exit Function
        End If
    End If
    
    If ilListIndex = CR_SPLITREGION Then
        If RptSelSR!rbcSortBy(0).Value = True Then      'station option
            If Not gOpenPrtJob("CopySplitByStn.Rpt") Then
                gGenReportSR = False
                Exit Function
            End If
        Else                                'splits option
            'If Not gOpenPrtJob("SplitRegion.Rpt") Then
            If Not gOpenPrtJob("CopySplit.Rpt") Then
                gGenReportSR = False
                Exit Function
            End If
        End If
