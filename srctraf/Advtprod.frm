...
Begin VB.Form AdvtProd 
   ...
   End
End
Attribute VB_Name = "AdvtProd"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of Advtprod.frm on Wed 6/17/09 @ 12:56 P
...
Dim smInitProd As String
Dim imMergeRan As Integer 'Merge arn, therefore ignore cancel so that list will be updated
Dim hmPrf As Integer    'Advertiser product file handle
...
Dim imFirstFocusProd As Integer
...
Dim imPrfRecLen As Integer        'ADF record length
...
'    ilSvIndex = cbcSelect.ListIndex
    ...
    If imTerminate Then
        Exit Sub
    End If
    If imFirstFocusProd Then 'Test if coming from sales source- if so, branch to first control
        imFirstFocusProd = False
        If igAdvtProdCallSource <> CALLNONE Then  'If from advt or contract- set name and branch to control
            If smInitProd = "" Then
                cbcSelect.ListIndex = 0
            Else
                cbcSelect.Text = smInitProd    'New name
            End If
            cbcSelect_Change
            If smInitProd <> "" Then
                'mSetCommands
                gFindMatch smInitProd, 1, cbcSelect
...
Private Sub cmcCancel_Click()
    If igAdvtProdCallSource <> CALLNONE Then
        ...
        If igAdvtProdCallSource = CALLSOURCECOLLECT Then
            igAdvtProdCallSource = CALLCANCELLED
        End If
        If igAdvtProdCallSource = CALLSOURCEFEED Then
            igAdvtProdCallSource = CALLCANCELLED
        End If
        If imMergeRan Then
...
Private Sub cmcErase_Click()
    ...
    If imSelectedIndex > 0 Then
        ...
        ilRet = gLLCodeRefExist(AdvtProd, tmPrf.lCode, "Fsf.Btr", "FsfPrfCode")
        If ilRet Then
            Screen.MousePointer = vbDefault
            slMsg = "Cannot erase - a Feed Spot references name"
            ilRet = MsgBox(slMsg, vbOKOnly + vbExclamation, "Erase")
            Exit Sub
        End If
        ilRet = gLLCodeRefExist(AdvtProd, tmPrf.lCode, "Phf.Btr", "PhfPrfCode")
        If ilRet Then
            Screen.MousePointer = vbDefault
            ...
            Exit Sub
        End If
        ilRet = gLLCodeRefExist(AdvtProd, tmPrf.lCode, "Rvf.Btr", "RvfPrfCode")
        If ilRet Then
            Screen.MousePointer = vbDefault
            ...
            Exit Sub
        End If
        ilRet = gLLCodeRefExist(AdvtProd, tmPrf.lCode, "Pjf.Btr", "PjfPrfCode")
        If ilRet Then
            Screen.MousePointer = vbDefault
        ...
        End If
        Screen.MousePointer = vbDefault
        ilRet = MsgBox("OK to remove " & tmPrf.sName, vbOKCancel + vbQuestion, "Erase")
        ...
        slStamp = gFileDateTime(sgDBPath & "Prf.Btr")
        ilRet = btrDelete(hmPrf)
        On Error GoTo cmcEraseErr
        gBtrvErrorMsg ilRet, "cmcErase_Click (btrDelete)", AdvtProd
        On Error GoTo 0
...
'            tmDsf.iRemoteID = tmPrf.iRemoteID
'            tmDsf.lAutoCode = tmPrf.lAutoCode
...
'        End If
        If imTerminate Then
            ...
            Exit Sub
        End If
        'If lbcAdvtProdCode.Tag <> "" Then
        '    If slStamp = lbcAdvtProdCode.Tag Then
        '        lbcAdvtProdCode.Tag = FileDateTime(sgDBPath & "Prf.Btr")
        '    End If
        'End If
        If sgProdCodeTag <> "" Then
            If slStamp = sgProdCodeTag Then
                sgProdCodeTag = gFileDateTime(sgDBPath & "Prf.Btr")
            End If
        End If
        'lbcAdvtProdCode.RemoveItem imSelectedIndex - 1
        gRemoveItemFromSortCode imSelectedIndex - 1, tgProdCode()
...
Private Sub cmcUpdate_Click()
    ...
    If Not imUpdateAllowed Then
        Exit Sub
    End If
    slName = Trim$(edcProd.Text)   'Save name
...
Private Sub edcProd_LostFocus()
    edcProd.Text = gRemoveIllegalPastedChar(edcProd.Text)
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set AdvtProd = Nothing   'Remove data segment
...
'
    ...
    'ilRet = gPopAdvtBox(AdvtProd, cbcAdvt, Traffic!lbcAdvertiser)
    ilRet = gPopAdvtBox(AdvtProd, cbcAdvt, tgAdvertiser(), sgAdvertiserTag)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mAdvtPopErr
        gCPErrorMsg ilRet, "mAdvtPop (gPopAdvtBox)", AdvtProd
        On Error GoTo 0
        If ilIndex > 1 Then
...
'
    ...
    Select Case ilBoxNo 'Branch on box type (control)
        Case NAMEINDEX 'Name
            ...
            edcProd.MaxLength = 35  'tgSpf.iAProd
            gMoveFormCtrl pbcProd, edcProd, tmCtrls(ilBoxNo).fBoxX, tmCtrls(ilBoxNo).fBoxY
...
'
    ...
    End If
    mInitBox
    AdvtProd.Height = cmcCancel.Top + 5 * cmcCancel.Height / 3
    gCenterStdAlone AdvtProd
    'AdvtProd.Show
    Screen.MousePointer = vbHourglass
'    mInitDDE

    imPopReqd = False
    imPrfRecLen = Len(tmPrf)  'Get and save PRF record length
    imBoxNo = -1 'Initialize current Box to N/A
    imChgMode = False
    imBSMode = False
    imFirstFocusName = True
    imFirstFocusProd = True
    imSelectedIndex = -1
...
'
    ...
    flTextHeight = pbcProd.TextHeight("1") - 35
    'Position panel and picture areas with panel
    plcProd.Move 1725, 780, pbcProd.Width + fgPanelAdj, pbcProd.Height + fgPanelAdj
    pbcProd.Move plcProd.Left + fgBevelX, plcProd.Top + fgBevelY
    'Name
...
'
    ...
    If (imAdvtSelectedIndex >= 0) And (tmPrf.iAdfCode = 0) Then
        slNameCode = tgAdvertiser(imAdvtSelectedIndex).sKey    'Traffic!lbcAdvertiser.List(imAdvtSelectedIndex)
        ilRet = gParseItem(slNameCode, 2, "\", slCode)
        On Error GoTo mMoveCtrlToRecErr
        gCPErrorMsg ilRet, "mMoveCtrlToRec (gParseItem field 2)", AdvtProd
        On Error GoTo 0
        slCode = Trim$(slCode)
        tmPrf.iAdfCode = CInt(slCode)
    End If
    If Not ilTestChg Or tmCtrls(NAMEINDEX).iChg Then
        tmPrf.sName = edcProd.Text
    End If
    Exit Sub
mMoveCtrlToRecErr:
    On Error GoTo 0
        ...
        If gLastFound(cbcSelect) <> -1 Then   'Name found
            If gLastFound(cbcSelect) <> imSelectedIndex Then
                If Trim$(edcProd.Text) = cbcSelect.List(gLastFound(cbcSelect)) Then
                    Beep
                    MsgBox "Advertiser Product name already defined, enter a different name", vbOKOnly + vbExclamation + vbApplicationModal, "Error"
                    edcProd.Text = Trim$(tmPrf.sName) 'Reset text
                    mSetShow imBoxNo
                    mSetChg imBoxNo
                    imBoxNo = 1
                    'mEnableBox imBoxNo
...
Private Sub mParseCmmdLine()
    ...
    'End If
    'gInitStdAlone AdvtProd, slStr, ilTestSystem
...
'
    ...
    ilRet = gPopAdvtProdBox(AdvtProd, ilCode, cbcSelect, tgProdCode(), sgProdCodeTag)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mPopulateErr
        gCPErrorMsg ilRet, "mPopulate (gIMoveListBox)", AdvtProd
        On Error GoTo 0
...
'
    ...
    Dim ilRet As Integer    'Return status
    slNameCode = tgProdCode(ilSelectIndex - 1).sKey    'lbcAdvtProdCode.List(ilSelectIndex - 1)
    ilRet = gParseItem(slNameCode, 2, "\", slCode)
    On Error GoTo mReadRecErr
    gCPErrorMsg ilRet, "mReadRecErr (gParseItem field 2)", AdvtProd
    On Error GoTo 0
    tmPrfSrchKey.lCode = CLng(slCode)
    ilRet = btrGetEqual(hmPrf, tmPrf, imPrfRecLen, tmPrfSrchKey, INDEXKEY0, BTRV_LOCK_NONE, ilForUpdate)
    On Error GoTo mReadRecErr
    gBtrvErrorMsg ilRet, "mReadRecErr (btrGetEqual: Advertiser Product)", AdvtProd
    On Error GoTo 0
    mReadRec = True
    Exit Function
mReadRecErr:
...
'
    ...
    Do  'Loop until record updated or added
        slStamp = gFileDateTime(sgDBPath & "Prf.btr")
        'If Len(lbcAdvtProdCode.Tag) > Len(slStamp) Then
        '    slStamp = slStamp & Right$(lbcAdvtProdCode.Tag, Len(lbcAdvtProdCode.Tag) - Len(slStamp))
        'End If
        If Len(sgProdCodeTag) > Len(slStamp) Then
            slStamp = slStamp & right$(sgProdCodeTag, Len(sgProdCodeTag) - Len(slStamp))
        End If
        If imSelectedIndex <> 0 Then
    ...
    gBtrvErrorMsg ilRet, slMsg, AdvtProd
    On Error GoTo 0
    ilRet = btrGetPosition(hmPrf, llRecPos)
    If (imSelectedIndex = 0) Then   'And (tgSpf.sRemoteUsers = "Y") Then 'New selected
        Do
            ...
            'slMsg = "mSaveRec (btrGetEqual:Advertiser Product Name)"
            'On Error GoTo mSaveRecErr
            'gBtrvErrorMsg ilRet, slMsg, AdvtProd
            'On Error GoTo 0
            tmPrf.iRemoteID = tgUrf(0).iRemoteUserID
    ...
    '    If slStamp = lbcAdvtProdCode.Tag Then
    '        lbcAdvtProdCode.Tag = FileDateTime(sgDBPath & "Prf.btr")
    '        If Len(slStamp) > Len(lbcAdvtProdCode.Tag) Then
    '            lbcAdvtProdCode.Tag = lbcAdvtProdCode.Tag & Right$(slStamp, Len(slStamp) - Len(lbcAdvtProdCode.Tag))
    '        End If
    '    End If
    'End If
    'If sgProdCodeTag <> "" Then
    '    If slStamp = sgProdCodeTag Then
    '        sgProdCodeTag = FileDateTime(sgDBPath & "Prf.btr")
    '        If Len(slStamp) > Len(sgProdCodeTag) Then
    '            sgProdCodeTag = sgProdCodeTag & Right$(slStamp, Len(slStamp) - Len(sgProdCodeTag))
    '        End If
    '    End If
    'End If
    'If imSelectedIndex <> 0 Then
    ...
    'slName = RTrim$(tmPrf.sName)
    'cbcSelect.AddItem slName
    'slName = tmPrf.sName + "\" + LTrim$(Str$(tmPrf.lCode))'slName + "\" + LTrim$(Str$(tmPrf.lCode))
    'lbcAdvtProdCode.AddItem slName
    'gAddItemToSortCode slName, tgProdCode(), True
...
'        mSetCommands
    ...
    Select Case ilBoxNo 'Branch on box type (control)
        Case NAMEINDEX 'Name
            gSetChgFlag tmPrf.sName, edcProd, tmCtrls(ilBoxNo)
    End Select
    mSetCommands
End Sub
'*******************************************************
...
'
    ...
    If (imAdvtSelectedIndex < 0) Or (cbcSelect.Text = "") Then
        ...
        Else
            cbcSelect.Enabled = False
        End If
        pbcProd.Enabled = False  'Disallow mouse
        pbcSTab.Enabled = False
        pbcTab.Enabled = False
    Else
        If (igWinStatus(ADVERTISERSLIST) = 1) And (Trim$(tgUrf(0).sName) <> sgCPName) And (Trim$(tgUrf(0).sName) <> sgSUName) Then
    ...
    End If
    If (Not ilAltered) And (imAdvtSelectedIndex >= 0) And (tgUrf(0).sMerge = "I") And (imUpdateAllowed) Then
...
'
    ...
    If (ilBoxNo = NAMEINDEX) Then
        slStr = gReplaceIllegalCharacters(edcProd.Text)
        edcProd.Text = slStr
    End If
    ...
    Select Case ilBoxNo 'Branch on box type (control)
        Case NAMEINDEX 'Name
            ...
            slStr = edcProd.Text
            gSetShow pbcProd, slStr, tmCtrls(ilBoxNo)
    End Select
End Sub
'*******************************************************
'*                                                     *
...
Private Sub pbcClickFocus_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    End If
End Sub
Private Sub pbcProd_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    Next ilBox
End Sub
Private Sub pbcProd_Paint()
    Dim ilBox As Integer
    For ilBox = imLBCtrls To UBound(tmCtrls) Step 1
        pbcProd.CurrentX = tmCtrls(ilBox).fBoxX + fgBoxInsetX
        pbcProd.CurrentY = tmCtrls(ilBox).fBoxY + fgBoxInsetY
        pbcProd.Print tmCtrls(ilBox).sShow
    Next ilBox
End Sub
...
Private Sub plcScreen_Paint()
    ...
    plcScreen.Print "Advertiser Product"
End Sub
