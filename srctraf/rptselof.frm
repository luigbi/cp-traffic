...
Begin VB.Form RptSelOF 
   Appearance      =   0  'Flat
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Order Fulfillment Selection"
   ClientHeight    =   5535
   ClientLeft      =   165
   ClientTop       =   1305
   ClientWidth     =   9270
   ...
   Begin VB.Frame frcOption 
      ...
      Begin VB.PictureBox pbcSelC 
         ...
         Begin V81TrafficReports.CSI_Calendar CSI_CalTo 
            ...
            CSI_DefaultDateType=   0
         End
         Begin VB.CheckBox ckcInclFills 
            Caption         =   "Include Fills"
            ForeColor       =   &H80000008&
            Height          =   259
            Left            =   120
            TabIndex        =   52
      ...
      Begin VB.PictureBox pbcOption 
         ...
         Begin VB.ListBox lbcSelection 
            ...
            Height          =   1500
            Index           =   4
            ItemData        =   "rptselof.frx":0000
            Left            =   120
            List            =   "rptselof.frx":0002
         ...
         Begin VB.ListBox lbcSelection 
            ...
            Height          =   1290
            Index           =   3
            ItemData        =   "rptselof.frx":0004
            Left            =   120
            List            =   "rptselof.frx":0006
            MultiSelect     =   2  'Extended
            TabIndex        =   49
            Top             =   2310
            Visible         =   0   'False
         ...
         Begin VB.ListBox lbcSelection 
            ...
            Height          =   1290
            Index           =   2
            ItemData        =   "rptselof.frx":0008
            Left            =   120
            List            =   "rptselof.frx":000A
            MultiSelect     =   2  'Extended
            TabIndex        =   48
            Top             =   2310
            Visible         =   0   'False
         ...
         Begin VB.ListBox lbcSelection 
            ...
            Height          =   1290
            Index           =   1
            ItemData        =   "rptselof.frx":000C
            Left            =   120
            List            =   "rptselof.frx":000E
            MultiSelect     =   2  'Extended
            TabIndex        =   35
            Top             =   2295
            Visible         =   0   'False
         ...
         Begin VB.ListBox lbcSelection 
            ...
            Height          =   1500
            Index           =   0
            ItemData        =   "rptselof.frx":0010
            Left            =   120
            List            =   "rptselof.frx":0012
            MultiSelect     =   2  'Extended
            TabIndex        =   34
            Top             =   300
            Width           =   3720
   ...
   End
End
Attribute VB_Name = "RptSelOF"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of rptselof.frm on Wed 6/17/09 @ 12:56 P
...
'
' File Name: RptSelOF.Frm - Order Fullfillment Report
'
'
' Release: 4.7  10/9/00
'
...
Private Sub CkcDiscrepOnly_Click()
    If CkcDiscrepOnly.Value Then
        ckcInclFills.Value = vbUnchecked
        ckcInclFills.Enabled = False
    Else
        ckcInclFills.Enabled = True
    End If
End Sub
...
Private Sub cmcGen_Click()
    ...
    For ilJobs = ilStartJobNo To ilNoJobs Step 1
        igJobRptNo = ilJobs
        If Not gOpenPrtJob("Fulfill.Rpt") Then
            igGenRpt = False
            ...
            Exit Sub
        End If
        ilRet = gCmcGenOF(imGenShiftKey, smLogUserCode)
        If ilRet = -1 Then
            igGenRpt = False
            frcOutput.Enabled = igOutput
            frcCopies.Enabled = igCopies
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set RptSelOF = Nothing   'Remove data segment
...
'
    ...
    ilRet = gPopAdvtBox(RptSelOF, lbcSelection, tgRptAdvertiserCode(), sgRptAdvertiserCodeTag)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mAdvtPopErr
        gCPErrorMsg ilRet, "mAdvtPop (gPopAdvtBox)", RptSelOF
        On Error GoTo 0
    End If
    Exit Sub
mAdvtPopErr:
...
'
    Dim ilRet As Integer
   ilRet = gPopAgyBox(RptSelOF, lbcSelection, tgRptAgencyCode(), sgRptAgencyCodeTag)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mAgencyPopErr
        gCPErrorMsg ilRet, "mAgencyPop (gPopAgyBox)", RptSelOF
        On Error GoTo 0
    End If
    Exit Sub
mAgencyPopErr:
...
Dim slStr As String
    ...
    RptSelOF.Caption = smSelectedRptName & " Report"
...
Private Sub mInitReport()
    ...
    mMnfPop "C", RptSelOF!lbcSelection(3), tgRptNameCode(), sgRptNameCodeTag    'Traffic!lbcSalesperson
...
'    edcSelCFrom1.Move 3240, 30
    ...
    'plcShow.Move 120, 3390         'selection of advt, agency, prod prot, slsp.  Default this to advt for selection
    rbcShow(1).Value = True
    If rbcShow(1).Value = True Then
        rbcShow_Click 1
    End If
...
Dim ilRet As Integer
    ...
    ilRet = gIMoveListBox(RptSelOF, lbcLocal, tlSortCode(), slSortCodeTag, "Mnf.Btr", gFieldOffset("Mnf", "MnfName"), 20, ilfilter(), slFilter(), ilOffSet())
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mMnfPopErr
        gCPErrorMsg ilRet, "mMnfPop (gImoveListBox)", RptSelOF
        On Error GoTo 0
    End If
    Exit Sub
mMnfPopErr:
...
Private Sub mParseCmmdLine()
    ...
    'End If
    'gInitStdAlone RptSelOF, slStr, ilTestSystem
    'If igStdAloneMode Then
    '    smSelectedRptName = "Order FulFillment"
...
Private Sub mSellConvVirtVehPop(ilIndex As Integer, ilUselbcVehicle As Integer)
    ...
    If ilUselbcVehicle Then
        ...
        ilRet = gPopUserVehicleBox(RptSelOF, VEHCONV_WO_FEED + VEHCONV_W_FEED + VEHSELLING + VEHVIRTUAL + ACTIVEVEH, lbcSelection(ilIndex), tgVehicle(), sgVehicleTag)
    Else
        ...
        ilRet = gPopUserVehicleBox(RptSelOF, VEHCONV_WO_FEED + VEHCONV_W_FEED + VEHSELLING + VEHVIRTUAL + ACTIVEVEH, lbcSelection(ilIndex), tgCSVNameCode(), sgVehicleTag)    'lbcCSVNameCode)
    End If
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mSellConvVirtVehPopErr
        gCPErrorMsg ilRet, "mSellConvVirtVehPop (gPopUserVehicleBox: Vehicle)", RptSelOF
        On Error GoTo 0
    End If
    Exit Sub
...
'
    ...
    ilEnable = False
    If (CSI_CalFrom.Text <> "") Then            '12-16-19 change to csi cal control
'    If (edcSelCFrom.Text <> "") Then
        ...
        If ilEnable Then
            ...
            If RptSelOF!rbcShow(0).Value Then        'show everything, no selection
                ilEnable = True
            Else
                If RptSelOF!rbcShow(1).Value Then   'advt selection
                    ilIndex = 1
...
'
    Dim ilRet As Integer
    'Repopulate if required-
    ilRet = gPopSalespersonBox(RptSelOF, 0, True, True, lbcSelection, tgRptSalespersonCode(), sgRptSalespersonCodeTag, igSlfFirstNameFirst)
    If ilRet <> CP_MSG_NOPOPREQ Then
        On Error GoTo mSPersonPopErr
        gCPErrorMsg ilRet, "mSPersonPop (gPopSalespersonBox)", RptSelOF
        On Error GoTo 0
    End If
    Exit Sub
mSPersonPopErr:
