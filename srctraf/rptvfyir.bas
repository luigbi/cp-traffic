Attribute VB_Name = "RptVfyIR"
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of rptvfyir.bas on Wed 6/17/09 @ 12:56 P
...
'*                                                     *
'*******************************************************
Function gCmcGenIR(ilGenShiftKey As Integer, slLogUserCode As String) As Integer
'******************************************************************************************
'* Note: VBC id'd the following unreferenced items and handled them as described:         *
'*                                                                                        *
'* Local Variables (Removed)                                                              *
'*  ilRet                         slNameCode                    ilMonth                   *
'*  ilStartYear                   ilEndYear                     slStartStd                *
'*  slEndStd                      llStartStd                    llEndStd                  *
'*                                                                                        *
'******************************************************************************************
...
'
    ...
    Dim llDate As Long
    Dim llInputEnd As Long
    'ilStartDateYear and ilEndDateYear must be the same year to process.
    Dim ilStartDateYear As Integer
    Dim ilEndDateYear As Integer
    Dim slTemp As String

    gCmcGenIR = 0
'    slDate = RptSelIR!edcSelCFrom.Text          'start date
     slDate = RptSelIR!CSI_CalFrom.Text          'start date    9-5-19 use csi calendar control vs edit box
    If Not gValidDate(slDate) Then
        mReset
        RptSelIR!CSI_CalFrom.SetFocus
        Exit Function
    End If
    'year of dates entered must be the same
    llDate = gDateValue(slDate)
    slDate = Format$(llDate, "m/d/yy")               'insure year is appended to month/day

    'get the year of the std months end date (for the start date entered)
    'i.e. if 1/15/08 entered, get the year of Januarys std month end date
    slTemp = gObtainEndStd(slDate)
    gObtainYearMonthDayStr slTemp, True, slYear, slMonth, slDay
    ilStartDateYear = Val(slYear)

'    slStr = RptSelIR!edcSelCFrom1.Text         'end date
    slStr = RptSelIR!CSI_CalTo.Text         'end date 9-5-19 use csi calendar control vs edit box
    If Not gValidDate(slStr) Then
        mReset
        RptSelIR!CSI_CalTo.SetFocus
        Exit Function
    End If

    'determine the year processing by the end date (since a std bdcst year must always be
    'with the same calendar year (last sun of the month)
    llInputEnd = gDateValue(slStr)
    'slStr = Format$(llInputEnd, "m/d/yy")

    slTemp = gObtainEndStd(slStr)
    'strip off the year
    gObtainYearMonthDayStr slTemp, True, slYear, slMonth, slDay
    ilEndDateYear = Val(slYear)

'    slStartStd = "1/15/" & Trim$(slYear)
'    slStartStd = gObtainStartStd(slStartStd)
'    llStartStd = gDateValue(slStartStd)
'
'    slEndStd = "12/15/" & Trim$(slYear)
'    slEndStd = gObtainEndStd(slEndStd)
'    llEndStd = gDateValue(slEndStd)


    'If (llDate >= llStartStd And llDate <= llEndStd) And (llInputEnd >= llStartStd And llInputEnd <= llEndStd) Then
    '1-6-09 The dates entered cannot span 2 different standard bdcst years since the Network Inventory table
    'is entered by the standard bdcst month/year
    If ilStartDateYear = ilEndDateYear Then
        'OK dates,
    Else
        MsgBox "Dates cannot span broadcast year"
        mReset
        RptSelIR!CSI_CalFrom.SetFocus
        Exit Function
    End If
    ...
    slStr = slDate & "-" & slStr
    If Not gSetFormula("UserDates", "'" & slStr & "'") Then
        gCmcGenIR = -1
        Exit Function
    End If

    If RptSelIR!rbcSelC2(0).Value Then            'gross or net
    ...
    If RptSelIR!rbcTotalsBy(0).Value Then            'detail vs summary
    ...
    slExclude = ""
    slInclude = ""
    gIncludeExcludeCkc RptSelIR!ckcSelC1(0), slInclude, slExclude, "Holds"
    gIncludeExcludeCkc RptSelIR!ckcSelC1(1), slInclude, slExclude, "Orders"

    gIncludeExcludeCkc RptSelIR!ckcSelC1(2), slInclude, slExclude, "Std"
    gIncludeExcludeCkc RptSelIR!ckcSelC1(3), slInclude, slExclude, "Reserve"
    gIncludeExcludeCkc RptSelIR!ckcSelC1(4), slInclude, slExclude, "Remnant"
    gIncludeExcludeCkc RptSelIR!ckcSelC1(5), slInclude, slExclude, "DR"
    gIncludeExcludeCkc RptSelIR!ckcSelC1(6), slInclude, slExclude, "PI"
    gIncludeExcludeCkc RptSelIR!ckcSelC1(7), slInclude, slExclude, "PSA"
    gIncludeExcludeCkc RptSelIR!ckcSelC1(8), slInclude, slExclude, "Promo"

    gIncludeExcludeCkc RptSelIR!ckcSelC1(9), slInclude, slExclude, "Trade"
...
Sub mReset()
    igGenRpt = False
    RptSelIR!frcOutput.Enabled = igOutput
    RptSelIR!frcCopies.Enabled = igCopies
    'RptSelIR!frcWhen.Enabled = igWhen
    RptSelIR!frcFile.Enabled = igFile
    RptSelIR!frcOption.Enabled = igOption
    'RptSelIR!frcRptType.Enabled = igReportType
    Beep
End Sub
