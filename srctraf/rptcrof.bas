Attribute VB_Name = "RPTCROF"
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of rptcrof.bas on Wed 6/17/09 @ 12:56 PM
'***** Mode: AutoSelect Standard Mode (Internal References Only)***************************
'******************************************************************************************
'* Note: VBC id'd the following unreferenced items and handled them as described:         *
'*                                                                                        *
'* Private Variables (Removed)                                                            *
'*  tmSmfSrchKey                  tmCxfSrchKey                                            *
'******************************************************************************************
...
'*******************************************************************
'*                                                                 *
'*      Procedure Name:gCreatOF                                    *
'*                                                                 *
'*             Created:7/20/06       By:D. Hosaka                  *
'*            Modified:              By:                           *
'*                                                                 *
'*            Comments: Generate Order Fulfillment                 *
'*          Show all spots or just outside/mg spots highlighting
'*          the spots ordered days, dates, times and where it is
'*          scheduled to be able to see what spots were ordered out
'*          of their ordered specs.
'*                                                                 *
'*         This routine was copied from the Daily Spot Report.
'*         Many of the options have been hidden but if reinstated  *
'*         much of the filtering is still in
'
'*                                                                 *
'*******************************************************************
Sub gCreateOF()
'
    ...
    tlCntTypes.iHold = gSetCheck(RptSelOF!ckcCType(0).Value)
    tlCntTypes.iOrder = gSetCheck(RptSelOF!ckcCType(1).Value)
    tlCntTypes.iNetwork = gSetCheck(RptSelOF!ckcCType(2).Value)
    tlCntTypes.iStandard = gSetCheck(RptSelOF!ckcCType(3).Value)
    tlCntTypes.iReserv = gSetCheck(RptSelOF!ckcCType(4).Value)
    tlCntTypes.iRemnant = gSetCheck(RptSelOF!ckcCType(5).Value)
    tlCntTypes.iDR = gSetCheck(RptSelOF!ckcCType(6).Value)
    tlCntTypes.iPI = gSetCheck(RptSelOF!ckcCType(7).Value)
    tlCntTypes.iPSA = gSetCheck(RptSelOF!ckcCType(8).Value)
    tlCntTypes.iPromo = gSetCheck(RptSelOF!ckcCType(9).Value)
    tlCntTypes.iTrade = gSetCheck(RptSelOF!ckcCType(10).Value)
    tlCntTypes.iMissed = gSetCheck(RptSelOF!ckcSpots(0).Value)
    tlCntTypes.iCharge = gSetCheck(RptSelOF!ckcSpots(1).Value)
    tlCntTypes.iZero = gSetCheck(RptSelOF!ckcSpots(2).Value)
    tlCntTypes.iADU = gSetCheck(RptSelOF!ckcSpots(3).Value)
    tlCntTypes.iBonus = gSetCheck(RptSelOF!ckcSpots(4).Value)
    tlCntTypes.iXtra = gSetCheck(RptSelOF!ckcSpots(5).Value)        'fill to show on inv
    tlCntTypes.iFill = gSetCheck(RptSelOF!ckcSpots(6).Value)        'fill to exclude on inv
    'the previous 2 options have been hidden, override with the fill question shown on the screen
    If RptSelOF!ckcInclFills.Value = vbChecked Then
        tlCntTypes.iXtra = True
        tlCntTypes.iFill = True
    Else
        tlCntTypes.iXtra = False
        tlCntTypes.iFill = False
    End If

    tlCntTypes.iOrphan = gSetCheck(RptSelOF!CkcDiscrepOnly.Value)       'discrepancies only (meaning only outsides & mgs)
    tlCntTypes.iNC = gSetCheck(RptSelOF!ckcSpots(7).Value)
    tlCntTypes.iRecapturable = gSetCheck(RptSelOF!ckcSpots(8).Value)
    tlCntTypes.iSpinoff = gSetCheck(RptSelOF!ckcSpots(9).Value)
    tlCntTypes.iFixedTime = True    'gSetCheck(RptSelOF!ckcRank(0).Value)
    tlCntTypes.iSponsor = True  'gSetCheck(RptSelOF!ckcRank(1).Value)
    tlCntTypes.iDP = True   'gSetCheck(RptSelOF!ckcRank(2).Value)
    tlCntTypes.iROS = True  'gSetCheck(RptSelOF!ckcRank(3).Value)
    'tlCntTypes.iRated = gSetCheck(RptSelOF!ckcEvents(0).Value)     'include program events
    'tlCntTypes.iNonRated = gSetCheck(RptSelOF!ckcEvents(1).Value)  'comments
    'tlCntTypes.iSuburban = gSetCheck(RptSelOF!ckcEvents(2).Value)  'show open avails only

    'build the spot lengths
    'tlCntTypes.iLenHL(0) = Val(RptSelOF!edcLength(0))
    'tlCntTypes.iLenHL(1) = Val(RptSelOF!edcLength(1))

    'if days of week to be selectable, add ckcDays array and controls test
    For ilLoop = 0 To 6
        tlCntTypes.iValidDays(ilLoop) = True
    Next ilLoop

    'get all the dates needed to work with
'    slDate = RptSelOF!edcSelCFrom.Text               'start date entred
    slDate = RptSelOF!CSI_CalFrom.Text               'start date entred
    llEffDate = gDateValue(slDate)
    llDateEntered = llEffDate               'save orig date to calculate week index

'    slDate = RptSelOF!edcSelCFrom1.Text               'end date entered
    slDate = RptSelOF!CSI_CalTo.Text               'end date entered
    If slDate = "" Then
    ...
    imSelList = 0
    If RptSelOF!rbcShow(1).Value Then   'advt selection
        imSelList = 1
    ElseIf RptSelOF!rbcShow(2).Value Then   'agy selection
        imSelList = 2
    ElseIf RptSelOF!rbcShow(3).Value Then   'product protection selection
        imSelList = 3
    Else
        imSelList = 4
    End If
    ...
    If imSelList > 0 Then
        If Not (RptSelOF!ckcAllOthers.Value = vbChecked) Then
            ilCkcAllOthers = True
            For ilLoop = 0 To RptSelOF!lbcSelection(imSelList).ListCount - 1 Step 1
                If (RptSelOF!lbcSelection(imSelList).Selected(ilLoop)) Then
    ...
    End If
    tmVef.iCode = 0
    For ilVehicle = 0 To RptSelOF!lbcSelection(0).ListCount - 1 Step 1
        If (RptSelOF!lbcSelection(0).Selected(ilVehicle)) Then
    ...
    Next ilVehicle                              'For ilvehicle = 0 To RptSelOF!lbcSelection(0).ListCount - 1
    ilRet = btrClose(hmCxf)
    ilRet = btrClose(hmRdf)
    ilRet = btrClose(hmSsf)
    ilRet = btrClose(hmSdf)
...
'*                                                               *
'*                                                               *
'*      Procedure Name:mBuildDetail                              *
'*                                                               *
'*                                                               *
'*  This code was duplicated from the Daily Spot report.  Some
'   code has not been cleaned up and is not used in the Fulfillment
'   report
'
'*      CBF fields required for the Fulfilment output (fulfill.rpt)
'       cbfgendate - generation date for record filter
'       cbfgentime - generation time for record filter
'       cbfvefCode - vehicle code
'       cbfDysTms - Times (using line overrides)
'       cbfDtFrstBkt - Spot scheduled date
'       cbfLength - spot length
'       cbfVehSort - original vehicle if spot mg or outside
'       cbfrdfDPSort - DP code (to test the avail inclusion/exclusion)
'                   for ordered spot
'       cbfSortField1 - rate ($ or descriptions like spinoff, adu, etc)
'       cbfSortField2 - days of week string or, daily # of spots
'       cbfadfCode - Advertiser code
'       cbfagfCode - Scheduled named avail code (anf)
'       cbfSTartDate - start date of week ordered
'       cbfEndDate - end date of week ordered
'       cbfStatus - F = fill, G = MG, O = outside, blank = normal scheduled spot
'       cbfTime - spot sched time
'       cbfClfLine - SDF Code
'*****************************************************************
...
'*  ilPass                        ilEndTime                     slTime                    *
'*  ilStartPass                   ilCreateAvail                 slXMid                    *
'*  ilFoundLLC                                                                            *
'******************************************************************************************
...
'
                ...
                '      ivefSort - if MG or OUT, orign vehicle missed
                '      RdfDPSort - Rate Card DP code
                        ...
                        If llTime >= lmSTime And llTime < lmETime Then      'event within entered time parameters
                            ...
                            For ilSpot = 1 To tmAvail.iNoSpotsThis Step 1
                               LSet tmSpot = tmSsf.tPas(ADJSSFPASBZ + ilEvt + ilSpot)
                                tmCbf.lLineNo = 0                   'init sdf code for debugging
                                ilSpotOK = True                             'assume spot is OK to include
                                ilSpotTypes = 0
                                ilCTypes = 0
                                ilRanks = 0

                                mFilterSpotTypes ilCTypes, ilSpotTypes, ilSpotOK, tlCntTypes     'determine to include/exclude trades, psa,promo, remnants, PI, DR & extra spots
                                ...
                                If ilSpotOK Then
                                    ...
                                    'tmCbf.lValue(1) = 0                 'init out of date specs flag
                                    'tmCbf.lValue(2) = 0                 'init out of  times flag
                                    tmCbf.lValue(0) = 0                 'init out of date specs flag
                                    tmCbf.lValue(1) = 0                 'init out of  times flag
                                                              tmCbf.iVehSort = 0                  'init orig vehicle moved from
                                    tmCbf.lLineNo = tmSdf.iLineNo
                                    tmCbf.iLen = tmSdf.iLen
                                    tmCbf.lRate = tlCntTypes.lRate
                                    tmCbf.iTime(0) = tmSdf.iTime(0)     'spot time
                                    tmCbf.iTime(1) = tmSdf.iTime(1)
                                    ...
                                    tmCbf.sStatus = ""          'regular scheduled spot
                                    ...
                                    tmCbf.lLineNo = tmSdf.lCode     'save the spot code for debugging
...
Dim ilFound As Integer
    ...
    'ilShowOVTimes = False
    tmCbf.sDysTms = " "
    tmCbf.iRdfDPSort = tmRdf.iCode              'DP code to show the Avail ordered
    ...
    Else
        ...
        Next ilLoop
        'see if the date spot sched is on one of the valid days to air from flight
    ...
    'get day of week to see if within the line specs.  If not, set out of day flag
    gUnpackDateLong tmSdf.iDate(0), tmSdf.iDate(1), llDate
    ilLoop = gWeekDayLong(llDate)
    If ilTempDays(ilLoop) = 0 Then
        'tmCbf.lValue(1) = 1                 'error, outside valid day of week
        tmCbf.lValue(0) = 1                 'error, outside valid day of week
    End If
    ...
    'Times
    'ilShowOVTimes = False
    gUnpackTimeLong tmSdf.iTime(0), tmSdf.iTime(1), False, llAirTime
    ...
    If tmSdf.lChfCode = 0 Then
        ...
        'convert time to long to test if air time within the override times
        gUnpackTimeLong ilTempStartTime(0), ilTempStartTime(1), False, llStartTime(0)
        gUnpackTimeLong ilTempEndTime(0), ilTempEndTime(1), True, llEndTime(0)
        ilShowOVTimes = True
    Else
        'Add times
...
'                'convert time to long to test if air time within the DP times
'                gUnpackTimeLong tmRdf.iStartTime(0, ilLoop), tmRdf.iStartTime(1, ilLoop), False, llStartTime
'                gUnpackTimeLong tmRdf.iEndTime(0, ilLoop), tmRdf.iEndTime(1, ilLoop), True, llEndTime
'Exit For
'            End If
'        Next ilLoop
        mGetLegalTimes llStartTime(), llEndTime()
    End If

    ilFound = False
    For ilLoop = LBound(llStartTime) To UBound(llStartTime) Step 1
        If (llAirTime >= llStartTime(ilLoop)) And (llAirTime < llEndTime(ilLoop)) Then
            slStartTime = gFormatTimeLong(llStartTime(ilLoop), "A", "1")
            slEndTime = gFormatTimeLong(llEndTime(ilLoop), "A", "1")
    ...
    If Not ilFound Then
        slStartTime = gFormatTimeLong(llStartTime(0), "A", "1")
        slEndTime = gFormatTimeLong(llEndTime(0), "A", "1")
         tmCbf.sDysTms = Trim$(slStartTime) & "-" & Trim$(slEndTime)
'        If llAirTime < llStartTime Or llAirTime > llEndTime Then
            'tmCbf.lValue(2) = 1                 'set flag for out of time
            tmCbf.lValue(1) = 1                 'set flag for out of time
'        End If
    End If
...
'    End If
    ...
    For ilLoop = LBound(llStartTime) To UBound(llStartTime) Step 1
        If (llTime >= llStartTime(ilLoop)) And (llTime < llEndTime(ilLoop)) Then
            ilTimeFound = True
            If (llEndTime(ilLoop) - llStartTime(ilLoop)) <= 900 Then          '15M
                ilFound = True
                ilRanks = &H1
                If Not tlCntTypes.iFixedTime Then
                    ilSpotOK = False
...
Sub mFilterSpotTypes(ilCTypes As Integer, ilSpotTypes As Integer, ilSpotOK As Integer, tlCntTypes As CNTTYPES)
    ...
    ElseIf (tmSpot.iRank And RANKMASK) = PERINQUIRYRANK Then
        ilCTypes = &H800
        If Not tlCntTypes.iPI Then
            ilSpotOK = False
        End If

    ElseIf (tmSpot.iRank And RANKMASK) = TRADERANK Then
        ilCTypes = &H400
        If Not tlCntTypes.iTrade Then
            ilSpotOK = False
        End If

    'ElseIf tmSpot.iRank = EXTRARANK Then
    '    ilSpotTypes = &H20
    '    If Not tlCntTypes.iXtra Then
    '        ilSpotOK = False
    '    End If

    ElseIf (tmSpot.iRank And RANKMASK) = PROMORANK Then
        ilCTypes = &H200
        If Not tlCntTypes.iPromo Then
            ilSpotOK = False
        End If

    ElseIf (tmSpot.iRank And RANKMASK) = PSARANK Then
        ilCTypes = &H100
...
Dim ilSelectedField(0 To 1) As Integer
    ...
    If tmSpot.lSdfCode = tmSdf.lCode And ilRet = BTRV_ERR_NONE Then
        ...
        If (tlCntTypes.iOrphan = True) Then         'include discreps only (vs all spots)
            If ((tmSdf.sSchStatus = "O" Or tmSdf.sSchStatus = "G") And (tmSdf.sSpotType <> "X")) Then       'discreps only (mgs & outsides)
            Else
            ilSpotOK = False
            End If
        End If
    ...
    '12-18-00 Show "M for N" if its multiple makegoods
        ...
        Do While (ilRet = BTRV_ERR_NONE) And (tmSmf.lChfCode = tmSdf.lChfCode) And (tmSmf.iLineNo = tmSdf.iLineNo) And (tmSmf.lFsfCode = tmSdf.lFsfCode)
            If (tmSmf.lSdfCode = tmSdf.lCode) Then
                ...
                tmCbf.iVehSort = tmSmf.iOrigSchVef    'orig vehicle code
                ...
                Do While ilDay <> 0
                    ...
                    ilDay = gWeekDayLong(llDate)
                Loop
                'week that the spot should have been scheduled in
                gPackDateLong llDate, tmCbf.iStartDate(0), tmCbf.iStartDate(1)
                gPackDateLong llDate + 6, tmCbf.iEndDate(0), tmCbf.iEndDate(1)
                'see if the spot is scheduled in correct week
                gUnpackDateLong tmSdf.iDate(0), tmSdf.iDate(1), llAirDate
                If llAirDate < llDate Or llAirDate > llDate + 6 Then
                    'tmCbf.lValue(1) = 1                 'error , outside week
                    tmCbf.lValue(0) = 1                 'error , outside week
                End If
        ...
        Loop
    ElseIf tmSdf.sSpotType = "X" Then          'fill
        'no dates on fill spot
        tmCbf.iStartDate(0) = 0
        tmCbf.iStartDate(1) = 0
        tmCbf.iEndDate(0) = 0
        tmCbf.iEndDate(1) = 0
    Else            'regular scheduled spot
        gUnpackDateLong tmSdf.iDate(0), tmSdf.iDate(1), llDate
        'backup to Monday
        ilDay = gWeekDayLong(llDate)
        Do While ilDay <> 0
...
Sub mGetLegalTimes(llStartTime() As Long, llEndTime() As Long)
    ...
    ilTBTime = 0
    For ilTimes = LBound(llStartTime) To UBound(llStartTime) Step 1
        llStartTime(ilTimes) = -1
        llEndTime(ilTimes) = -1
    Next ilTimes
