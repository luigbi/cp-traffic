...
Begin VB.Form RptSelEx 
   ...
   End
End
Attribute VB_Name = "RptSelEx"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************************************************
'***** VB Compress Pro 6.11.32 generated this copy of rptselex.frm on Wed 6/17/09 @ 12:56 P
...
'
' File Name: RptSelEx.Frm   Dump Text file report
'
' Release: 4.7
'
' Description:
...
Private Sub cmcGen_Click()
    ...
    ilStartJobNo = 1
    ilNoJobs = 1
    ilListIndex = igRptType
    For ilJobs = ilStartJobNo To ilNoJobs Step 1
        igJobRptNo = ilJobs
        If Not mGenReportEX() Then
            igGenRpt = False
       ...
       '1 falls thru - successful crystal report
        ...
        Else
            ...
            ilRet = gExportCRW(slFileName, imFTSelectedIndex)   '6-12-02
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set RptSelEx = Nothing   'Remove data segment
...
Private Sub mInit()
    ...
    If ilRet = 0 Then
        ...
        gAutomationAlertAndLogHandler "Unable to open print engine", vbOkOnly, "mInit"
        mTerminate False
        imTerminate = -99
        Exit Sub
    End If
    ...
    imAllClicked = False
    imSetAll = True
    'gCenterStdAlone RptSelEx
    'RptSelEx.Move -90, -90, 30, 30      'make form small and out of the way so its not seen
    RptSelEx.Move -330, -330, 30, 30      'make form small and out of the way so its not seen
...
Private Sub mInitReport()
    ...
    mSetCommands
    Screen.MousePointer = vbDefault
'    gCenterModalForm RptSelEx
End Sub
'************************************************************
'*                                                          *
'*      Procedure Name:mParseCmmdLine                       *
'*                                                          *
'*             Created:5/22/01       By:D. Hosaka           *
'*            Modified:              By:                    *
'*                                                          *
'*            Comments: Parse command line                  *
'*                                                          *
'*            Create reports for Crystal reports and dump   *
'*                                                          *
'************************************************************
Private Sub mParseCmmdLine()
    ...
    '    slCommGenDate = "5/28/01"
    '    slCommGenTime = "7:57:06AM"
    '    slDisPlayPrint = "0"    '0 = display, 1= print
    '    slReportType = "1"        'Station feed report
    '    slReportCallType = "STATIONFEEDJOB"
    '    slVefCode as string
    '    'Mandatory parms are 1st and 2nd :
    '    '1st parm:  function coming from (Logs^Test (or Prod or NoHelp)
    '    '2nd parm:  user name
    '    '3rd parm:  Rpt Call type (STATIONFEEDJOB)
    '    '4th parm:  Report type (1)
    '    '5th parm:  Output (0 = display, 1=print)
    '    '6th parm:  index of "Save To" type 6-12-02
    '    '7th parm:  Name of "Saved to" file  6-12-02
    '    '8th parm:  Gen Date   6-12-02
    '    '9th parm:  Gen Time   6-12-02
    '     10th parm: vehicle    11-4-06
    '    slCommand = "Logs^Test\Guide\" & slReportCallType & "\" & slReportType & "\" & slDisPlayPrint & "\" & slCommGenDate & "\" & slCommGenTime & "\" & Str$(ilVefCode)
    '    imShowHelpmsg = False
    'Else
    ...
    'End If
    'gInitStdAlone RptSelEx, slStr, ilTestSystem

    ilRet = gParseItem(slCommand, 3, "\", slStr)                'Task or Job:Report call type
    igRptCallType = Val(Trim$(slStr))
    ilRet = gParseItem(slCommand, 4, "\", slStr)                'Report within task or job
    igRptType = Val(Trim$(slStr))
    ilRet = gPopExportTypes(cbcFileType)         '10-19-01
    ilRet = gParseItem(slCommand, 5, "\", slStr)            'diplay or print
    igOutputTo = Val(Trim$(slStr))

    If igOutputTo = 0 Then                          'display
        rbcOutput(0).Value = True
        rbcOutput(1).Value = False
        rbcOutput(2).Value = False
    ...
    ilRet = gParseItem(slCommand, 6, "\", slStr)                'Index to save to type
    imFTSelectedIndex = Val(Trim$(slStr))   '6-12-02
    ilRet = gParseItem(slCommand, 7, "\", slStr)               'Save File Name
    edcFileName.Text = Trim$(slStr)         '6-12-02

    ilRet = gParseItem(slCommand, 8, "\", slStr)                'Gen Date
    gPackDate slStr, igGenDate(0), igGenDate(1)
    ilRet = gParseItem(slCommand, 9, "\", slStr)                'Gen Time
    gPackTime slStr, igGenTime(0), igGenTime(1)
    gUnpackTimeLong igGenTime(0), igGenTime(1), False, lgGenTime
    ilRet = gParseItem(slCommand, 10, "\", slStr)               'vehicle
    imVefCode = Val(slStr)
    ilRet = gParseItem(slCommand, 11, "\", slStr)               'date
    smEncoDate = Trim$(slStr)               'date to gather (1 day at a time)
End Sub
'*******************************************************
'*                                                     *
...
Private Function mCmcGenEx() As Integer
...
Dim slEncoMonth As String
Dim slEncoDay As String
Dim slEncoYear As String
Dim ilVpfIndex As Integer
Dim slZoneSelection As String
Dim ilLoop As Integer


    ilVpfIndex = -1
    ilVpfIndex = gVpfFind(ExptEnco, imVefCode)              'determine vehicle options index
    slZoneSelection = ""
    If ilVpfIndex >= 0 Then
        'For ilLoop = 1 To 4
        For ilLoop = 0 To 3
            If tgVpf(ilVpfIndex).sGFed(ilLoop) = "*" Then               'this has a feed, use it
                If slZoneSelection = "" Then
                    slZoneSelection = " and ({ODF_One_Day_Log.odfZone} = '" & Trim$(tgVpf(ilVpfIndex).sGZone(ilLoop)) & "'"
                Else
                    slZoneSelection = slZoneSelection & " or {ODF_One_Day_Log.odfZone} = '" & Trim$(tgVpf(ilVpfIndex).sGZone(ilLoop)) & "'"
                End If
            End If
        Next ilLoop
        If slZoneSelection <> "" Then
            slZoneSelection = slZoneSelection & ")"
        End If
    End If

    slSelection = ""
    gObtainYearMonthDayStr smEncoDate, True, slEncoYear, slEncoMonth, slEncoDay
    gUnpackDate igGenDate(0), igGenDate(1), slDate
    gObtainYearMonthDayStr slDate, True, slYear, slMonth, slDay
    gUnpackTime igGenTime(0), igGenTime(1), "A", "1", slTime
    ...
    slSelection = slSelection & " and {ODF_One_Day_Log.odfVefCode} =  " & Trim$(str(imVefCode))
    slSelection = slSelection & " and {ODF_One_Day_Log.odfType} =  4"
    slSelection = slSelection & " and {ODF_One_Day_Log.odfAirDate} = Date(" & slEncoYear & "," & slEncoMonth & "," & slEncoDay & ")"
    slSelection = slSelection & slZoneSelection
...
Private Function mGenReportEX() As Integer

    If Not gOpenPrtJob("EncoFeedLog.Rpt") Then
        mGenReportEX = False
        Exit Function
    End If
