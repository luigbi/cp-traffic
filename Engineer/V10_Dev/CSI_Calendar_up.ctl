VERSION 5.00
Begin VB.UserControl CSI_Calendar_UP 
   ...
   ScaleHeight     =   4830
   ScaleWidth      =   5400
   ToolboxBitmap   =   "CSI_Calendar_up.ctx":0000
   Begin VB.CommandButton btn_DownArrow 
      ...
      Width           =   300
   End
   Begin VB.PictureBox pb_DayNameFontHolder 
      ClipControls    =   0   'False
      Height          =   330
      Left            =   330
      ScaleHeight     =   270
      ...
      Width           =   990
   End
   Begin VB.PictureBox pb_CalFrame 
      Appearance      =   0  'Flat
      BorderStyle     =   0  'None
      ClipControls    =   0   'False
      ForeColor       =   &H80000008&
   ...
   End
End
Attribute VB_Name = "CSI_Calendar_UP"
...
Enum CalDateTypeList_UP
   LongMonth
   ShortMonth
End Enum

Public Enum CalDefaultDateList_UP
   csiNoDate_UP
   csiCurrentDate_UP
   csiThisMonday_UP
   csiNextMonday_UP
   csiStandardBroadcastMonday_UP
End Enum
...
Private imCalDateFormat As CalDateTypeList_UP
...
Private bmDefaultDateType As CalDefaultDateList_UP
...
Private Sub UserControl_Initialize()
    ...
    ' imCalDateFormat = LongMonth
    ' bmIsDroppedDown = False
    Call PositionAllControls

    'bmIsDroppedDown = True
    
    pb_CalFrame.Visible = False
...
Private Sub PositionAllControls()
    ...
    For ilLoop = 0 To 6
        lc_WeekNames(ilLoop).Font = pb_DayNameFontHolder.Font
        lc_WeekNames(ilLoop).FontBold = pb_DayNameFontHolder.FontBold
        lc_WeekNames(ilLoop).FontSize = pb_DayNameFontHolder.FontSize
        lc_WeekNames(ilLoop).FontItalic = pb_DayNameFontHolder.FontItalic
    Next

    For ilLoop = 0 To NDAYS
        KeyPad_Buttons(ilLoop).Font = pb_DayNameFontHolder.Font
        KeyPad_Buttons(ilLoop).FontBold = pb_DayNameFontHolder.FontBold
        KeyPad_Buttons(ilLoop).FontSize = pb_DayNameFontHolder.FontSize
        KeyPad_Buttons(ilLoop).FontItalic = pb_DayNameFontHolder.FontItalic
    Next

    iPixelWidth = pb_DayNameFontHolder.TextWidth("00") + (Screen.TwipsPerPixelX * 1)
    iPixelHeight = pb_DayNameFontHolder.TextHeight("00")
    ...
    For ilLoop = 0 To 6
        lc_WeekNames(ilLoop).Top = btn_MonthPrev.Height + 60
    ...
    ilCurTop = btn_MonthPrev.Height + 60 + lc_WeekNames(0).Height
    ilCurLeft = ilLeftBorder
    For ilLoop = 0 To NDAYS
    ...
    ilTotalHeight = ilCurTop + ilButtonHeights

    pb_CalFrame.Top = 0
    pb_CalFrame.Left = 0
    pb_CalFrame.Width = KeyPad_Buttons(6).Left + KeyPad_Buttons(6).Width + ilRightBorder - Screen.TwipsPerPixelX
    pb_CalFrame.Height = ilTotalHeight + (Screen.TwipsPerPixelX * 4)
    
    ec_InputBox.Font = Font
    ec_InputBox.FontSize = FontSize
    ec_InputBox.FontBold = FontBold
    ec_InputBox.FontItalic = FontItalic
    ec_InputBox.Top = pb_CalFrame.Height
    btn_DownArrow.Height = ec_InputBox.Height - (Screen.TwipsPerPixelX * 5)
    ...
    ec_Year.Width = pb_DayNameFontHolder.TextWidth("888888")
    ...
    imDropDownWidth = pb_CalFrame.Width
    imDropDownHeight = ec_InputBox.Height + pb_CalFrame.Height
    bmIgnoreResize = False
End Sub
...
Private Sub pb_CalFrame_Paint()
    pb_CalFrame.ForeColor = RGB(0, 0, 0)
    pb_CalFrame.Line (0, 0)-Step(pb_CalFrame.Width - Screen.TwipsPerPixelX, pb_CalFrame.Height - Screen.TwipsPerPixelY), , B
End Sub
...
Private Sub ec_InputBox_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    End Select
End Sub
Private Sub pb_CalFrame_KeyDown(KeyCode As Integer, Shift As Integer)
...
Private Sub mSetCalendarDays(sDate As String)
    ...
    'If bmIsDroppedDown Then
        ...
        ' This date is not on this calendar.
            'dmCurDate = DateAdd("m", 1, dmCurDate)  ' Move forward 1 month.
...
Err_mSetCalendarDays:
    MsgBox "ERROR: mSetCalendarDays, Date = " & sDate
    Resume Next
End Sub
...
Private Sub UserControl_Resize()
    ...
    If ec_InputBox.BorderStyle = 0 Then
        ...
        btn_DownArrow.Height = ec_InputBox.Height
...
Private Sub btn_DownArrow_Click()
    ...
    If bmIsDroppedDown Then
        'Width = ec_InputBox.Width
        pb_CalFrame.Visible = False
        ' Height = pb_CalFrame.Height + ec_InputBox.Height
        ec_InputBox.SetFocus
        bmIsDroppedDown = False
        Exit Sub
    End If
    ...
    ' pb_CalFrame.Top = (ec_InputBox.Top + ec_InputBox.Height)

    pb_CalFrame.Left = ScaleLeft

    If imEditBoxAlignment = 1 Then  ' Are we aligning the edit box on the right ?
        If pb_CalFrame.Left + pb_CalFrame.Width < ec_InputBox.Left + ec_InputBox.Width Then
            ' Reposition the drop down so it always lines up on the right side.
            pb_CalFrame.Left = (ec_InputBox.Left + ec_InputBox.Width) - pb_CalFrame.Width
        End If
    End If
    pb_CalFrame.Visible = True
    pb_CalFrame.SetFocus

    If Width < pb_CalFrame.Width Then
        Width = pb_CalFrame.Width
    End If
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    Set pb_DayNameFontHolder.Font = fmDayNameFont
    Call PositionAllControls
End Sub
...
'
'****************************************************************************
Public Property Get TrueHeight() As Long
   TrueHeight = ec_InputBox.Height + pb_CalFrame.Height
End Property
Public Property Let SetEditBoxHeight(ilHeight As Integer)
    ec_InputBox.Height = ilHeight
    btn_DownArrow.Height = ec_InputBox.Height
    Call PositionAllControls
    PropertyChanged "SetEditBoxHeight"
End Property
...
Public Property Set CSI_DayNameFont(ByVal New_Font As Font)
    ...
    pb_DayNameFontHolder.Font = fmDayNameFont
    pb_DayNameFontHolder.FontName = fmDayNameFont.Name
    pb_DayNameFontHolder.FontSize = fmDayNameFont.Size
    pb_DayNameFontHolder.FontBold = fmDayNameFont.Bold
    pb_DayNameFontHolder.FontItalic = fmDayNameFont.Italic
...
'
'****************************************************************************
Public Property Get CSI_CalDateFormat() As CalDateTypeList_UP
   CSI_CalDateFormat = imCalDateFormat
End Property
Public Property Let CSI_CalDateFormat(Setting As CalDateTypeList_UP)
...
'
'****************************************************************************
Public Property Get CSI_DefaultDateType() As CalDefaultDateList_UP
   CSI_DefaultDateType = bmDefaultDateType
End Property
Public Property Let CSI_DefaultDateType(Setting As CalDefaultDateList_UP)
...
Public Function CSI_GetEditBoxTop() As Integer
    CSI_GetEditBoxTop = ec_InputBox.Top
End Function
