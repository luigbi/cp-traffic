Attribute VB_Name = "EngrBUZIPSUBS"
'******************************************************************************************
...
Public Function gContDBPathResolved() As Integer
    ...
    If sgServerDatabase = "" And btrIsANetworkPath(sgDBPath) = True Then
        MsgBox "ServerDatabase is not defined in Engineer.ini file." & Chr(10) & Chr(13) _
...
Public Sub gOpenBUMsgFile(sErrStr As String)
    ...
    End If
    On Error GoTo 0
    Print #hmBUMsg, "** " & sErrStr & Format$(gNow(), "ddddd") & " at " & Format$(gNow(), "ttttt") & " **"
    Close hmBUMsg
    Exit Sub
gOpenBUMsgFileErr:
    ilRet = Err.Number
