VERSION 5.00
Begin VB.Form frmStationMSAMktInfo 
   Caption         =   "MSA Market/Cluster Information"
   ClientHeight    =   4050
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8775
   Icon            =   "AffStationMSAMktInfo.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   4050
   ...
   End
End
Attribute VB_Name = "frmStationMSAMktInfo"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'******************************************************
'*  frmStationMSAMktInfo
'*
...
ErrHand:
    Screen.MousePointer = vbDefault
    gHandleError "AffErrorLog.txt", "frmStationMSAMktInfo-cboMarket_Change"
    imInChg = False
End Sub
...
Private Sub Form_Initialize()
    gSetFonts frmStationMSAMktInfo
    gCenterForm frmStationMSAMktInfo
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    ilRet = gPopMarkets()
    Set frmStationMSAMktInfo = Nothing
...
Private Function mFillMktInfo(slFromSaveName As String)
    ...
    If (frmStation!cbcMSAMarket.ListIndex <= 0) And (imIsFirstTime) Then
        cboMarket.ListIndex = 0
    Else
        If imIsFirstTime Then
            slName = Trim$(frmStation!cbcMSAMarket.GetName(frmStation!cbcMSAMarket.ListIndex))
        End If
        'If imSave Then
...
ErrHand:
    Screen.MousePointer = vbDefault
    gHandleError "AffErrorLog.txt", "frmStationMSAMktInfo-mFillMktInfo"
    imInChg = False
End Function
...
Private Function mSave()
    ...
    If (cboMarket.Text = "[New]") Or (Trim$(cboMarket.Text) = "") Then
        ...
        SQLQuery = SQLQuery & "(metName, metRank, metUstCode, metGroupName) "
        ...
        If gSQLWaitNoMsgBox(SQLQuery, False) <> 0 Then
            ...
            'GoSub ErrHand:
            Screen.MousePointer = vbDefault
            gHandleError "AffErrorLog.txt", "StationMSAMktInfo-mSave"
            imInChg = False
            mSave = False
            Exit Function
        End If
    ...
    Else
        SQLQuery = "UPDATE met"
        SQLQuery = SQLQuery & " SET metName = '" & txtName.Text & "',"
        SQLQuery = SQLQuery & "metRank = '" & Trim$(txtRank.Text) & "',"
...
ErrHand:
    Screen.MousePointer = vbDefault
    gHandleError "AffErrorLog.txt", "frmStationMSAMktInfo-mSave"
    imInChg = False
End Function
...
ErrHand:
    ...
    If (Err.Number <> 0) And (gMsg = "") Then
        gMsg = "A general error has occured in StationMSAMktInfo-mMktChkForChange: "
...
ErrHand:
    ...
    gMsg = ""
    If (Err.Number <> 0) And (gMsg = "") Then
        gMsg = "A general error has occured in StationMSAMktInfo-mTrimAndFixQuotes: "
        gMsgBox gMsg & Err.Description & "; Error #" & Err.Number, vbCritical
    End If
    imInChg = False
End Sub
...
Private Sub mBindControls()
    ...
    Else
        ...
        smName = txtName.Text
        txtRank.Text = Trim$(mkt_rst!metRank)
        smRank = txtRank.Text
...
ErrHand:
    Screen.MousePointer = vbDefault
    gHandleError "AffErrorLog.txt", "frmStationMSAMktInfo-mBindControls"
    imInChg = False
End Sub
